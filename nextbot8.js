...
const INITIAL_FRAMES = [
    ...
    'https://rabstvonext.ru/?vk_access_token_settings=&vk_app_id=51556925&vk_are_notifications_enabled=0&vk_is_app_user=1&vk_is_favorite=0&vk_language=ru&vk_platform=&vk_ref=other&vk_ts=1708687412&vk_user_id=728784826&sign=0zRKdZlLt76YMoUKHNqaAJRvcovR6bCYB8Oq2DbttxI',
    'https://rabstvonext.ru/?vk_access_token_settings=&vk_app_id=51556925&vk_are_notifications_enabled=0&vk_is_app_user=1&vk_is_favorite=0&vk_language=ru&vk_platform=web_external&vk_ref=other&vk_ts=1708687577&vk_user_id=730177498&sign=1FVCt3eJQXmy3xYuJ8OcvZyrkec_PZKb5SEDYrTp4PY',
    'https://rabstvonext.ru/?vk_access_token_settings=&vk_app_id=51556925&vk_are_notifications_enabled=0&vk_is_app_user=0&vk_is_favorite=0&vk_language=ru&vk_platform=web_external&vk_ref=other&vk_ts=1708687666&vk_user_id=320332&sign=OkbALDrV_PrUEhdSLew37vpZ9Z1bkCn4TW_MyPBxbGw',
...
function sess(userId) {
    if (!sessions[userId])
        sessions[userId] = { accIdx: 0, victim: null, delay: 5 }
...
class GameAccount {
    ...
    async load() {
        ...
        try {
            const urlObj = new URL(this.iframe)
            this.userId = parseInt(urlObj.searchParams.get('vk_user_id'))
        } catch (_) {}
    ...
    async collectMoney() {
        return this.call('collectmoney', {
            sign_data: {
                sign: "A1nHesGIXg_jxPT6uWNp7lH0DZohJ6z7Q67sSOC4QVE",
                ts: Math.floor(Date.now() / 1000)
            }
        })
    }

    // Упрощённый getProfile – без offset/count
    getProfile(userId) {
        const params = {}
...
async function buyNormal(userId, peerId, accIdx, victimId, delay, minPrice = 0, maxPrice = Infinity) {
    ...
    try {
        ...
        const myProfile = await ga.getProfile()
        const myInfo = myProfile?.item || {}
        await send(peerId,
            `🎯 Начинаю закуп | ${myInfo.name || 'Неизвестно'} ${myInfo.last_name || ''} | Баланс: ${(myInfo.money_osn || 0).toLocaleString('ru')} | Рабов: ${myInfo.rabs_count || 0}`
        )
        const profile = await ga.getProfile(victimId)
        const slaves = profile?.item?.rabs || []
        ...
        for (const s of slaves) {
            ...
            if (s.first_price < minPrice || s.first_price > maxPrice) {
                await send(peerId, `⏭ Пропущен ${s.name} ${s.last_name}: цена ${s.first_price.toLocaleString('ru')} вне диапазона`)
                continue
            }
            try {
...
async function buyMulti(userId, peerId, victimId, delay, minPrice = 0, maxPrice = Infinity) {
    ...
    try {
        ...
        if (!accList.length) return send(peerId, '❌ Нет аккаунтов')

        const accounts = []
        for (const [i, accData] of accList.entries()) {
            const ga = new GameAccount(accData)
            await ga.load()
            const p = await ga.getProfile()
            const info = p?.item || {}
            accounts.push({
                ga,
                idx: i,
                name: `${info.name || 'Неизвестно'} ${info.last_name || ''}`,
                balance: info.money_osn || 0,
                slavesCount: info.rabs_count || 0
            })
        }

        const accListMsg = accounts.map(a => 
            `👤 #${a.idx+1} ${a.name} | Баланс: ${a.balance.toLocaleString('ru')} | Рабов: ${a.slavesCount}`
        ).join('\n')
        await send(peerId,
            `🎯 Мульти-закупка | Аккаунтов: ${accounts.length}\n${accListMsg}\n` +
            `💰 Фильтр цен: ${minPrice} - ${maxPrice === Infinity ? '∞' : maxPrice} руб.\n▶️ Старт...`
        )

        const ga0 = accounts[0].ga
        ...
        for (const s of slaves) {
            if (job.stopped) break
            if (s.first_price < minPrice || s.first_price > maxPrice) {
                await send(peerId, `⏭ Пропущен ${s.name} ${s.last_name}: цена ${s.first_price.toLocaleString('ru')} вне фильтра`)
                continue
            }

            const acc = accounts.find(a => a.balance >= s.first_price)
            if (!acc) {
                await send(peerId, `⚠️ Нет аккаунтов с балансом >= ${s.first_price.toLocaleString('ru')} для раба ${s.name} ${s.last_name}. Остановка.`)
                break
            }

            try {
                const res = await acc.ga.buyRab(s.id_vk)
                if (res?.res) {
                    bought++; spent += s.first_price
                    acc.balance -= s.first_price
                    await send(peerId,
                        `✅ Куплен: ${s.name} ${s.last_name}\n` +
                        `💵 Цена: ${s.first_price.toLocaleString('ru')} руб.\n` +
                        `📈 Доход: +${s.first_zarabotok}/час\n` +
                        `📦 Всего куплено: ${bought} | Остаток на акк #${acc.idx+1}: ${acc.balance.toLocaleString('ru')} руб.`
                    )
                }
            } catch (_) {}
            await sleep(delay)
...
// Основная функция с подгрузкой страниц
async function buyPremium(userId, peerId, victimId, delay, limit, minPrice = 0, maxPrice = Infinity) {
    ...
    try {
        ...
        for (const acc of accounts) {
            ...
            // Цикл покупок на одном аккаунте, может запрашивать новые страницы
            while (accBought < limit && !job.stopped) {
                // Получаем актуальную страницу рабов жертвы
                const fresh = await acc.ga.getProfile(victimId)
                const slaves = fresh?.item?.rabs || []
                if (!slaves.length) {
                    await send(peerId, `❌ Больше нет рабов у жертвы. Завершаю.`)
                    job.stopped = true
                    break
                }

                // Обрабатываем рабов на текущей странице
                for (const s of slaves) {
                    if (job.stopped) break
                    if (accBought >= limit) break

                    if (s.first_price < minPrice || s.first_price > maxPrice) {
                        await send(peerId, `⏭ Пропущен ${s.name} ${s.last_name}: цена ${s.first_price} вне фильтра`)
                        continue
                    }

                    if (acc.balance < s.first_price) {
                        await send(peerId, `⚠️ Аккаунт #${acc.idx+1}: недостаточно средств для ${s.name} (цена: ${s.first_price.toLocaleString('ru')}, баланс: ${acc.balance.toLocaleString('ru')}). Пропускаю, ищу дальше...`)
                        continue
                    }
                    ...
                    try {
                        ...
                        if (res?.res) {
                            ...
                            await send(peerId,
                                ...
                                `💵 Цена: ${s.first_price.toLocaleString('ru')} руб.\n` +
                                `📈 Доход: +${s.first_zarabotok}/час\n` +
                                `📦 Акк #${acc.idx+1}: ${accBought}/${limit} | Всего: ${totalBought} | Остаток: ${acc.balance.toLocaleString('ru')} руб.`
                            )
                        } else {
                            await sleep(500)
                        }
                ...
                // Если страница кончилась и не достигнут лимит, цикл while продолжит – снова запросит fresh страницу
            }

            await send(peerId, `✅ Акк #${acc.idx+1} завершил: куплено ${accBought} (лимит ${limit})`)
        }
...
async function upgradeRabs(userId, peerId, accIdx, minPrice = 0, maxPrice = Infinity) {
    ...
    try {
        ...
        if (!rabs.length) { await send(peerId, '❌ У вас нет рабов для прокачки'); return }
        await send(peerId,
        ...
        for (const rab of rabs) {
            ...
            if (rab.upgrade_price && (rab.upgrade_price < minPrice || rab.upgrade_price > maxPrice)) {
                await send(peerId, `⏭ Пропущен ${rab.name} ${rab.last_name}: цена прокачки ${rab.upgrade_price} вне фильтра`)
                continue
            }
            try {
    ...
    try {
        ...
        for (let i = startIdx; i < endIdx; i++) {
            try {
                ...
                const profile = await ga.getProfile()
                const info = profile?.item || {}
                const acc = accs[i]
                const text = [
                    `👤 Аккаунт #${i + 1}`,
                    `🔗 ${acc.frameUrl ? 'Фрейм: ' + acc.frameUrl : 'Токен'}`,
...
async function handle(userId, peerId, text) {
    ...
    if (cmd === '/delay') {
        ...
        if (isNaN(ms) || ms < 5) return send(peerId, '❌ Минимум 5 мс')
        s.delay = ms
    ...
    if (cmd === '/collect') {
        ...
        try {
            const ga = makeAcc(s.accIdx, userId); await ga.load()
            const before = await ga.getProfile()
            const beforeBalance = before?.item?.money_osn || 0
            const r = await ga.collectMoney()
            const after = await ga.getProfile()
            const afterBalance = after?.item?.money_osn || 0
            const diff = afterBalance - beforeBalance
            if (r?.res) {
                await send(peerId, `💰 Собрано: ${diff.toLocaleString('ru')} руб. (было ${beforeBalance.toLocaleString('ru')}, стало ${afterBalance.toLocaleString('ru')})`)
            } else {
                await send(peerId, `❌ ${r?.message || 'Нечего собирать'}`)
            }
        } catch (e) { await send(peerId, `❌ Ошибка: ${e.message}`) }
        return
    }
    ...
    if (cmd === '/collectall') {
        ...
        for (let i = 0; i < getAccounts(userId).length; i++) {
            try {
                ...
                const diff = afterBalance - beforeBalance
                results.push(r?.res
                    ? `✅ Акк #${i + 1}: +${diff.toLocaleString('ru')} руб.`
