...
async function initDB() {
  ...
  const columnsToAdd = [
    { name: 'stamina', type: 'INT DEFAULT 100' },
    { name: 'pickaxe_lvl', type: 'INT DEFAULT 1' },
    { name: 'drills', type: 'INT DEFAULT 0' },
    { name: 'oil_rigs', type: 'INT DEFAULT 0' },
    { name: 'last_work', type: 'BIGINT DEFAULT 0' },
    { name: 'last_rest', type: 'BIGINT DEFAULT 0' },
    { name: 'last_drill', type: 'BIGINT DEFAULT 0' },
    { name: 'last_oil', type: 'BIGINT DEFAULT 0' },
    { name: 'guild_id', type: 'INT DEFAULT NULL' },
    { name: 'last_black', type: 'BIGINT DEFAULT 0' },
    { name: 'jail_until', type: 'BIGINT DEFAULT 0' },
    { name: 'has_roof', type: 'BOOLEAN DEFAULT FALSE' },
    { name: 'last_steal', type: 'BIGINT DEFAULT 0' },
    { name: 'last_daily', type: 'BIGINT DEFAULT 0' },
    { name: 'hidden', type: 'BOOLEAN DEFAULT FALSE' },
    { name: 'banned', type: 'BOOLEAN DEFAULT FALSE' },
    { name: 'virus', type: 'INT DEFAULT 0' },
    { name: 'virus_end', type: 'BIGINT DEFAULT 0' },
    { name: 'vaccine_end', type: 'BIGINT DEFAULT 0' },
    { name: 'player_class', type: 'VARCHAR(32) DEFAULT NULL' },
    { name: 'is_admin', type: 'BOOLEAN DEFAULT FALSE' },
    { name: 'work_count', type: 'INT DEFAULT 0' },
    { name: 'craft_level', type: 'INT DEFAULT 1' },
    { name: 'drills_enhanced', type: 'INT DEFAULT 0' },
    { name: 'drills_diamond', type: 'INT DEFAULT 0' },
    { name: 'battle_rating', type: 'INT DEFAULT 0' },
    { name: 'referral_code', type: 'VARCHAR(16) UNIQUE' },
    { name: 'referred_by', type: 'BIGINT DEFAULT NULL' },
  ];
  
  for (const col of columnsToAdd) {
    try {
      await pool.query(`ALTER TABLE players ADD COLUMN IF NOT EXISTS ${col.name} ${col.type}`);
    } catch (e) {
      // Колонка уже существует
    }
  }
  
  // Добавляем столбец deputy_id в guilds
  try {
...
// Проверка, может ли игрок брать из казны (глава или зам)
async function canTakeFromTreasury(uid, guildId) {
  const gr = await pool.query('SELECT owner_id, deputy_id FROM guilds WHERE id=$1', [guildId]);
  if (!gr.rows[0]) return false;
  const g = gr.rows[0];
  return Number(g.owner_id) === uid || Number(g.deputy_id) === uid;
}

// ═══════════════════════════════════════════════════════════
//  ИСПРАВЛЕНИЕ: toLocaleString для форматирования баланса
// ═══════════════════════════════════════════════════════════
function fmt(num) {
...
async function cmdSellOre(ctx, uid, oreName) {
  ...
  if (oreName === 'всё' || oreName === 'все') {
    ...
    for (const [name, ore] of Object.entries(ORES)) {
      ...
      if (qty > 0) {
        ...
        parts.push(`${ore.emoji} ${name} x${qty} = ${fmt(sum)} монет`);
      }
    }
    ...
    if (contQty > 0) {
      ...
      parts.push(`🕵️ контрабанда x${contQty} = ${fmt(sum)} монет`);
    }
    if (total === 0)
  ...
  await pool.query(`UPDATE players SET balance=balance+$1 WHERE user_id=$2`, [sum, uid]);
  await send(ctx,
    `${ore.emoji} Продано: ${oreName} x${qty} по ${fmt(ore.price)} монет\n✅ +${fmt(sum)} монет`
  );
}
...
async function cmdPickaxe(ctx, uid) {
  ...
  if (p.balance < cur.cost)
    return send(ctx,
      ...
      `💰 Стоимость: ${fmt(cur.cost)} монет\n` +
      `👛 Твой баланс: ${fmt(p.balance)} монет\n` +
      `❌ Не хватает: ${fmt(cur.cost - p.balance)} монет.`
    );
...
async function cmdBuyDrill(ctx, uid, qty) {
  ...
  if (p.balance < totalCost)
    return send(ctx,
      `⚙️ Покупка ${qty} бур(ов) — ${fmt(totalCost)} монет\n` +
      `📈 Пассивный доход: +${DRILL_INCOME} монет/час каждый\n` +
      `👛 Твой баланс: ${fmt(p.balance)} монет\n` +
      `❌ Не хватает: ${fmt(totalCost - p.balance)} монет.`
    );
  ...
  if (pendingEarned > 0) msg += `💰 Автоматически собрано ${fmt(pendingEarned)} монет с буров.\n`;
...
async function cmdBuyOil(ctx, uid, qty) {
  ...
  if (p.balance < totalCost)
    return send(ctx,
      `🛢️ Покупка ${qty} вышки(ек) — ${fmt(totalCost)} монет\n` +
  ...
  if (pendingEarned > 0) msg += `💰 Автоматически собрано ${fmt(pendingEarned)} монет с вышек.\n`;
...
async function cmdIncome(ctx, uid) {
  ...
  if (info.activeDrills === 0 && info.activeEnhanced === 0 && info.activeDiamond === 0)
    return send(ctx, `⚙️ У тебя нет активных буров.\nКупи бур командой «Бур [кол-во]» за ${fmt(DRILL_COST)} монет/шт. или скрафти «Крафт бур обычный»`);
  ...
  let msg = `⚙️ Доход с буров!\n💰 +${fmt(earned)} монет (за ${(Math.min((now - p.last_drill) / 3_600_000, PASSIVE_CAP_HOURS)).toFixed(1)} ч.)\n`;
  if (info.activeDrills > 0) {
    let inc = DRILL_INCOME;
    if (info.engineerBonus > 0) inc = Math.floor(inc * (1 + info.engineerBonus));
    msg += `• Обычные буры: ${info.activeDrills}/${info.totalDrills} активны (+${fmt(info.activeDrills * inc)}/ч)\n`;
  }
  if (info.activeEnhanced > 0) {
    let inc = 100;
    if (info.engineerBonus > 0) inc = Math.floor(inc * (1 + info.engineerBonus));
    msg += `• Усиленные буры: ${info.activeEnhanced}/${info.totalEnhanced} активны (+${fmt(info.activeEnhanced * inc)}/ч)\n`;
  }
  if (info.activeDiamond > 0) {
    let inc = 250;
    if (info.engineerBonus > 0) inc = Math.floor(inc * (1 + info.engineerBonus));
    msg += `• Алмазные буры: ${info.activeDiamond}/${info.totalDiamond} активны (+${fmt(info.activeDiamond * inc)}/ч)\n`;
  }
...
async function cmdOilIncome(ctx, uid) {
  ...
  if (info.activeRigs === 0)
    return send(ctx, `🛢️ У тебя нет активных нефтяных вышек.\nКупи командой «Нефть [кол-во]» за ${fmt(OIL_COST)} монет/шт. или скрафти «Крафт вышка нефтяная»`);
  ...
  let msg = `🛢️ Доход с ${info.activeRigs}/${info.totalRigs} вышки(ок)!\n💰 +${fmt(earned)} монет (за ${(Math.min((now - p.last_oil) / 3_600_000, PASSIVE_CAP_HOURS)).toFixed(1)} ч.)\n📈 Доход: ${fmt(info.oilIncomePerHour)} монет/час`;
...
async function cmdProfile(ctx, uid) {
  ...
  // ИСПРАВЛЕНИЕ: форматирование всех чисел в пассивной информации
  if (info.totalDrills > 0) {
    let inc = info.engineerBonus > 0 ? Math.floor(DRILL_INCOME * (1 + info.engineerBonus)) : DRILL_INCOME;
    passiveInfo += `\n⚙️ Обычные буры: ${info.activeDrills}/${info.totalDrills} активны (+${fmt(info.activeDrills * inc)}/ч, накоп: ${fmt(info.drillPending)} монет)`;
  }
  if (info.totalEnhanced > 0) {
    let inc = info.engineerBonus > 0 ? Math.floor(100 * (1 + info.engineerBonus)) : 100;
    passiveInfo += `\n🔩 Усиленные буры: ${info.activeEnhanced}/${info.totalEnhanced} активны (+${fmt(info.activeEnhanced * inc)}/ч, накоп: ${fmt(info.drillPending)} монет)`;
  }
  if (info.totalDiamond > 0) {
    let inc = info.engineerBonus > 0 ? Math.floor(250 * (1 + info.engineerBonus)) : 250;
    passiveInfo += `\n💎 Алмазные буры: ${info.activeDiamond}/${info.totalDiamond} активны (+${fmt(info.activeDiamond * inc)}/ч, накоп: ${fmt(info.drillPending)} монет)`;
  }
  if (info.totalDrills === 0 && info.totalEnhanced === 0 && info.totalDiamond === 0) {
    passiveInfo += `\n⚙️ Буры: нет`;
  }
  if (info.totalRigs > 0) {
    passiveInfo += `\n🛢️ Вышки: ${info.activeRigs}/${info.totalRigs} активны (+${fmt(info.oilIncomePerHour)}/ч, накоп: ${fmt(info.oilPending)} монет)`;
  } else {
    passiveInfo += `\n🛢️ Вышки: нет`;
  }
  ...
  const lines = [
    `👤 Профиль: ${name}`,
    `🆔 ID: ${uid}`,
    `💰 Баланс: ${fmt(p.balance)} монет`,  // ИСПРАВЛЕНИЕ: форматирование баланса
...
async function cmdInfo(ctx, uid, targetId) {
  if (!await hasInfoAccess(uid)) {
    return send(ctx, '❌ У тебя нет доступа к команде «Инфа».\nДоступ выдаётся администратором: !датьинфу [ID] [1/7/30]');
  }
  ...
  let guildName = 'Нет';
  if (p.guild_id) {
    const gr = await pool.query('SELECT name, level FROM guilds WHERE id=$1', [p.guild_id]);
    if (gr.rows[0]) guildName = `${gr.rows[0].name} [Ур. ${gr.rows[0].level}]`;
  }

  const oreInfo = [];
  for (const [oreName, ore] of Object.entries(ORES)) {
    const qty = await getItemQty(targetId, oreName);
    if (qty > 0) oreInfo.push(`${ore.emoji}${oreName}: ${qty} шт.`);
  }
  const contQty = await getItemQty(targetId, 'контрабанда');
  if (contQty > 0) oreInfo.push(`🕵️контрабанда: ${contQty} шт.`);
  ...
  let passiveInfo = '';
  if (info.totalDrills > 0) passiveInfo += `\n⚙️ Обычные буры: ${info.activeDrills}/${info.totalDrills} активны`;
  if (info.totalEnhanced > 0) passiveInfo += `\n🔩 Усиленные буры: ${info.activeEnhanced}/${info.totalEnhanced} активны`;
  if (info.totalDiamond > 0) passiveInfo += `\n💎 Алмазные буры: ${info.activeDiamond}/${info.totalDiamond} активны`;
  if (info.totalRigs > 0) passiveInfo += `\n🛢️ Вышки: ${info.activeRigs}/${info.totalRigs} активны`;
  ...
  let healthInfo = '';
  if (p.vaccine_end > now) {
    healthInfo = `\n🦠 Защищён 💉 (${fmtTime(p.vaccine_end - now)})`;
  } else if (p.virus === 1 && p.virus_end > now) {
    healthInfo = `\n🦠 Болен 🤒 (ещё ${fmtTime(p.virus_end - now)})`;
  } else {
    healthInfo = `\n🦠 Здоров ✅`;
  }

  let classInfo = p.player_class ? `\n👤 Класс: ${p.player_class}` : `\n👤 Класс: не выбран`;
  const peaceInfo = await isInPeaceMode(targetId) ? `\n🕊️ В режиме мира` : '';
...
async function cmdInfoGuild(ctx, uid, guildName) {
  if (!await hasInfoAccess(uid)) {
    return send(ctx, '❌ У тебя нет доступа к команде «Инфа клан».\nДоступ выдаётся администратором: !датьинфу [ID] [1/7/30]');
  }

  const gr = await pool.query('SELECT * FROM guilds WHERE name=$1', [guildName]);
  if (!gr.rows[0]) return send(ctx, `❌ Бригада «${guildName}» не найдена.`);
  ...
  const memberCount = Number(memberR.rows[0].cnt);
  const guildLevel = Number(g.level);
  const treasury = Number(g.treasury);
  const hp = await getGuildHP(g.id);
  const nextCost = guildLevel * 5000;

  const topMembers = await pool.query(
    `SELECT user_id, balance FROM players WHERE guild_id=$1 AND banned=FALSE ORDER BY balance DESC LIMIT 5`,
    [g.id]
  );

  const totalWealth = await pool.query(
    `SELECT COALESCE(SUM(balance), 0) as total FROM players WHERE guild_id=$1 AND banned=FALSE`,
    [g.id]
  );

  let deputyInfo = '';
  if (Number(g.deputy_id)) {
    const deputyName = await getName(Number(g.deputy_id));
    deputyInfo = `\n👔 Зам. главы: ${deputyName} (ID: ${g.deputy_id})`;
  }

  const lines = [
    `🔍 ИНФОРМАЦИЯ О БРИГАДЕ «${g.name}»`,
    `👑 Глава: ${ownerName} (ID: ${g.owner_id})`,
    deputyInfo,
    `📊 Уровень: ${guildLevel}`,
    `❤️ HP клана: ${fmt(hp)}`,
    `💰 Казна: ${fmt(treasury)} монет`,
    `💰 Общий баланс участников: ${fmt(Number(totalWealth.rows[0].total))} монет`,
    `👤 Участников: ${memberCount}`,
    `📈 XP: ${Number(g.xp)}/${fmt(nextCost)} (до ур. ${guildLevel + 1})`,
    `⚔️ Очки войны: ${Number(g.war_points || 0)}`,
    `\n🏆 ТОП-5 УЧАСТНИКОВ:`,
  ];

  for (let i = 0; i < topMembers.rows.length; i++) {
    const m = topMembers.rows[i];
    const mName = await getName(Number(m.user_id));
    lines.push(`${i + 1}. ${mName} — ${fmt(Number(m.balance))}💰`);
  }

  await send(ctx, lines.join('\n'));
}

async function cmdAdminGiveInfo(ctx, uid, targetIdStr, daysStr) {
  if (!await isAdmin(uid)) {
    return send(ctx, '❌ Только администратор может выдавать доступ к «Инфа».');
  }

  const targetId = parseInt(targetIdStr);
  const days = parseInt(daysStr);

  if (!targetId || !days || ![1, 7, 30].includes(days)) {
    return send(ctx, '❌ Использование: !датьинфу [ID] [1/7/30]\n1 — на день\n7 — на неделю\n30 — на месяц');
  }
  ...
  const accessUntil = Date.now() + durationMs;
  
  globalThis.infoAccess.set(targetId, accessUntil);

  const targetName = await getName(targetId);
  const daysText = days === 1 ? '1 день' : days === 7 ? '7 дней' : '30 дней';

  await send(ctx,
    `✅ Доступ к «Инфа» выдан!\n` +
    `👤 Игрок: ${targetName} (ID: ${targetId})\n` +
    `⏳ Срок: ${daysText}\n` +
    `📅 До: ${new Date(accessUntil).toLocaleString('ru-RU')}\n\n` +
    `Доступные команды:\n` +
    `• Инфа [ID/@юзер] — просмотр профиля игрока\n` +
    `• Инфа клан [название] — просмотр информации о бригаде`
  );

  try {
    await vk.api.messages.send({
      user_id: targetId,
      message: `🔍 Тебе выдан доступ к команде «Инфа» на ${daysText}!\n\nИспользуй:\n• Инфа [ID] — детальный профиль игрока\n• Инфа клан [название] — информация о бригаде`,
      random_id: Math.floor(Math.random() * 1e9),
    });
  } catch {}
}

async function cmdAdminCheckInfo(ctx, uid, targetIdStr) {
  if (!await isAdmin(uid)) {
    return send(ctx, '❌ Только администратор может проверять доступ.');
  }

  const targetId = parseInt(targetIdStr);
  if (!targetId) return send(ctx, '❌ Укажи ID: !инфадоступ [ID]');

  const access = globalThis.infoAccess.get(targetId);
  if (!access || access <= Date.now()) {
    return send(ctx, `❌ У игрока #${targetId} нет доступа к «Инфа».`);
  }

  const targetName = await getName(targetId);
  await send(ctx,
    `🔍 Доступ к «Инфа» у игрока ${targetName} (ID: ${targetId}):\n` +
    `⏳ Активен до: ${new Date(access).toLocaleString('ru-RU')}\n` +
    `⏳ Осталось: ${fmtTime(access - Date.now())}`
  );
}
...
async function cmdTop(ctx) {
  ...
  for (let i = 0; i < res.rows.length; i++) {
    ...
    lines.push(`${i + 1}. [id${res.rows[i].user_id}|${name}] (@id${res.rows[i].user_id}) — ${fmt(Number(res.rows[i].balance))} монет`);
  }
  await send(ctx, lines.join('\n'));
}
...
async function cmdGuildTop(ctx) {
  const res = await pool.query(`
    ...
    GROUP BY g.id, g.name, g.level, g.treasury, g.xp, g.owner_id, g.deputy_id
  ...
  for (let i = 0; i < res.rows.length; i++) {
    ...
    lines.push(
      `${placeEmoji} ${g.name} [Ур. ${level}] | ❤️HP: ${fmt(hp)}`,
      `   👑 Глава: ${ownerName}${deputyStr}`,
      `   👤 Участников: ${members}`,
      `   💰 Общий баланс: ${fmt(totalWealth)} монет`,
      `   🏦 Казна: ${fmt(treasury)} монет`,
      `   ⚙️ Всего буров: ${totalDrills} | 🛢️ Всего вышек: ${totalRigs}`,
      `   ⛏️ Средний уровень кирки: ${avgPickaxe}`,
      `   🏆 Богатейший участник: ${richestName}`,
      `   📊 XP: ${Number(g.xp)}/${fmt(level * 5000)}`,
      ''
    );
  }
...
async function cmdShop(ctx) {
  ...
  for (const [name, item] of Object.entries(SHOP_ITEMS))
    lines.push(`• ${name} — ${fmt(item.cost)} монет\n  └ ${item.desc}`);
  lines.push(
    '',
    '🔧 Техника:',
    `• Кирка — улучшение (+30% за ур.) → «Кирка»`,
    `• Бур — ${fmt(DRILL_COST)} монет/шт., +${DRILL_INCOME} монет/час → «Бур [кол-во]»`,
    `• Нефтяная вышка — ${fmt(OIL_COST)} монет/шт., +${OIL_INCOME} монет/час → «Нефть [кол-во]»`,
    '',
    '⛏️ Руды (добываются через «Работа»):',
    `• 🪨 Уголь — ${fmt(ORES['уголь'].price)} монет/шт.`,
    `• ⚙️ Железо — ${fmt(ORES['железо'].price)} монет/шт.`,
    `• 💎 Алмаз — ${fmt(ORES['алмаз'].price)} монет/шт.`,
    `• 🥇 Золото — ${fmt(ORES['золото'].price)} монет/шт.`,
    `• 🔴 Рубин — ${fmt(ORES['рубин'].price)} монет/шт.`,
    `• 🪙 Платина — ${fmt(ORES['платина'].price)} монет/шт.`,
    '',
    '🕵️ Нелегальное:',
    `• Контрабанда — ${fmt(CONTRABAND_PRICE)} монет/шт. (добывается через «Чёрная работа»)`,
    '',
    '🦠 Медицина:',
    `• Аптечка — ${fmt(MEDKIT_COST)} монет (лечит вирус + 30 стамины)`,
    `• Вакцина — ${fmt(VACCINE_COST)} монет (защита от вируса на 24 часа)`,
    `• Антибиотик — ${fmt(ANTIBIOTIC_COST)} монет (ускоряет выздоровление в 2 раза)`,
    '',
    ...
    '',
    '⚔️ PvP:',
    `• Динамит для сноса — ${fmt(DESTROY_COST)}💰 (в магазине)`,
    `• Перемирие — ${fmt(PEACE_COST)}💰 (защита на 12 часов)`,
    '',
...
async function cmdBuyItem(ctx, uid, item, qty) {
  ...
  if (p.balance < totalCost)
    return send(ctx,
      `❌ Нужно ${fmt(totalCost)} монет (${fmt(shopItem.cost)} × ${qty}).\n👛 Твой баланс: ${fmt(p.balance)} монет.`
    );
  ...
  if (item === 'крыша') {
    ...
    return send(ctx,
      ...
      `💰 Списано: ${fmt(shopItem.cost)} монет.`
    );
  }
  ...
  if (item === 'антибиотик') {
    if (p.virus !== 1 || p.virus_end <= Date.now())
  ...
  await send(ctx, `✅ Куплено: «${item}» ×${qty} за ${fmt(totalCost)} монет!`);
}
...
async function cmdInventory(ctx, uid) {
  ...
  for (const r of res.rows) {
    if (ORES[r.item]) {
      oreItems.push(`• ${ORES[r.item].emoji} ${r.item}: ${r.quantity} шт. (= ${fmt(r.quantity * ORES[r.item].price)} монет)`);
    } else if (r.item === 'контрабанда') {
      otherItems.push(`• 🕵️ контрабанда: ${r.quantity} шт. (= ${fmt(r.quantity * CONTRABAND_PRICE)} монет)`);
...
async function cmdCraft(ctx, uid, craftName) {
  ...
  let recipeKey = null;
  const normalized = craftName.toLowerCase().trim();
  
  if (normalized === 'пластина' || normalized === 'железная пластина' || normalized === 'железную пластину') {
    recipeKey = 'пластина';
  } else if (normalized.includes('бур обычный') || normalized === 'обычный бур' || normalized === 'обычный_бур') {
    recipeKey = 'бур_обычный';
  } else if (normalized.includes('бур усиленный') || normalized === 'усиленный бур' || normalized === 'усиленный_бур') {
    recipeKey = 'бур_усиленный';
  } else if (normalized.includes('бур алмазный') || normalized === 'алмазный бур' || normalized === 'алмазный_бур') {
    recipeKey = 'бур_алмазный';
  } else if (normalized.includes('вышка') || normalized === 'нефтяная вышка' || normalized === 'нефтяную вышку') {
    recipeKey = 'вышка_нефтяная';
  } else if (normalized === 'шестерня' || normalized === 'шестерню') {
    recipeKey = 'шестерня_крафт';
  } else if (normalized.includes('алмазный наконечник') || normalized === 'наконечник') {
    recipeKey = 'алмазный_наконечник_крафт';
  } else if (normalized === 'магнит' || normalized === 'магнитный стабилизатор') {
    recipeKey = 'магнит_крафт';
  }
  ...
  if (p.balance < recipe.coins_cost) {
    return send(ctx,
      `❌ Не хватает монет: ${fmt(recipe.coins_cost)}💰\n` +
  ...
  msg += `💰 Потрачено: ${fmt(recipe.coins_cost)} монет\n`;
...
async function cmdSteal(ctx, uid, targetIdStr) {
  ...
  if (!topRes.rows.length)
    return send(ctx,
      ...
      `Для кражи нужен игрок с балансом от ${fmt(STEAL_MIN_BALANCE)} монет.\n` +
      `Проверь топ: «Топ»`
    );
  ...
  if (targetIdStr) {
    ...
    if (!isInTop5)
      return send(ctx,
        ...
        `Воровать можно только у игроков из топ-5 с балансом от ${fmt(STEAL_MIN_BALANCE)} монет.\n` +
        `Проверь топ: «Топ»`
      );
  ...
  await send(ctx,
    ...
    `💰 Украдено: ${fmt(stolen)} монет (${(pct * 100).toFixed(1)}% баланса, лимит ${fmt(STEAL_MAX_AMOUNT)}💰)\n` +
...
async function cmdHeal(ctx, uid, targetIdStr) {
  ...
  if (target.virus !== 1) {
    ...
    return send(ctx, `✅ Ты здоров! Лечение не требуется.\n💉 Для защиты купи «Вакцина» за ${fmt(VACCINE_COST)} монет.`);
  }
  ...
  if (p.balance < healCost)
    return send(ctx,
      `🏥 Лечение стоит ${fmt(healCost)} монет.\n` +
      `👛 Твой баланс: ${fmt(p.balance)} монет.\n` +
      `❌ Не хватает: ${fmt(healCost - p.balance)} монет.`
    );
  ...
  if (isHealingOther) {
    ...
    await send(ctx,
      ...
      `💰 Списано: ${fmt(healCost)} монет (50% скидка врача).`
    );
...
async function cmdVaccinate(ctx, uid) {
  ...
  if (p.balance < VACCINE_COST)
    return send(ctx,
      `💉 Вакцина стоит ${fmt(VACCINE_COST)} монет.\n` +
      `👛 Твой баланс: ${fmt(p.balance)} монет.\n` +
      `❌ Не хватает: ${fmt(VACCINE_COST - p.balance)} монет.`
    );
  ...
  await send(ctx,
    `💉 Ты вакцинирован!\n` +
    `🛡️ Защита от вируса на 24 часа.\n` +
    `💰 Списано: ${fmt(VACCINE_COST)} монет.`
  );
}
...
async function cmdVirusStatus(ctx, uid) {
  ...
  let healInfo = '';
  if (p.player_class === 'врач') {
    healInfo = `\n\n🏥 Ты врач! Можешь лечить других за ${fmt(Math.floor(MEDKIT_COST / 2))}💰:\n«Лечить [ID]»`;
  }
  ...
  await send(ctx,
    ...
    `${status}` +
    `${epidemicInfo}\n\n` +
    `🏥 Лечение: «Лечить» (${fmt(MEDKIT_COST)}💰) или «Купить аптечка»\n` +
    `💉 Вакцина: «Вакцина» или «Купить вакцина» (${fmt(VACCINE_COST)}💰)\n` +
    `🧪 Антибиотик: «Купить антибиотик» (${fmt(ANTIBIOTIC_COST)}💰)` +
    `${healInfo}`
  );
}
...
async function cmdEpidemicInfo(ctx) {
  ...
  await send(ctx,
    ...
    `🏥 Лечиться: «Лечить» (${fmt(MEDKIT_COST)}💰)\n` +
    `💉 Защита: «Вакцина» (${fmt(VACCINE_COST)}💰)\n` +
    `🧪 Ускорить: «Купить антибиотик» (${fmt(ANTIBIOTIC_COST)}💰)\n\n` +
    `👨‍⚕️ Врачи могут лечить других за ${fmt(Math.floor(MEDKIT_COST / 2))}💰: «Лечить [ID]»`
  );
}
...
async function cmdGuildInfo(ctx, uid) {
  ...
  if (!p.guild_id)
    return send(ctx,
      ...
      `Создать: «Создать бригаду [имя]» (${fmt(GUILD_COST)} монет)\n` +
  ...
  await send(ctx, [
    ...
    `💰 Казна: ${fmt(Number(g.treasury))} монет`,
    `📈 XP: ${Number(g.xp)}/${fmt(nextCost)} (до ур. ${Number(g.level) + 1})`,
...
async function cmdCreateGuild(ctx, uid, name) {
  ...
  if (p.balance < GUILD_COST)
    return send(ctx,
      `❌ Создание бригады стоит ${fmt(GUILD_COST)} монет.\n👛 Твой баланс: ${fmt(p.balance)} монет.`
    );
...
async function cmdTreasury(ctx, uid, amountStr) {
  ...
  if (!p.guild_id) return send(ctx, '❌ Ты не в бригаде.');
  if (p.balance < amount)
    return send(ctx, `❌ Недостаточно монет. Баланс: ${fmt(p.balance)} монет.`);
  ...
  if (gXp >= nextCost) {
    ...
    await send(ctx,
      `💰 +${fmt(amount)} монет в казну!\n` +
      `📈 XP: ${gXp}/${fmt(nextCost)} (до ур. ${gLevel + 1})`
    );
  }
}

// ИСПРАВЛЕНИЕ: доступ к казне для замов
async function cmdTakeFromTreasury(ctx, uid, amountStr) {
  ...
  // ИСПРАВЛЕНИЕ: проверка — глава ИЛИ зам
  if (!await canTakeFromTreasury(uid, p.guild_id))
    return send(ctx, '❌ Только глава бригады или его заместитель может снимать монеты из казны.');

  const treasuryAmount = Number(g.treasury);
  if (treasuryAmount < amount)
    return send(ctx,
      `❌ В казне недостаточно монет.\n` +
      `💰 В казне: ${fmt(treasuryAmount)} монет\n` +
      `Запрошено: ${fmt(amount)} монет`
    );
  ...
  await send(ctx,
    `💰 Ты снял ${fmt(amount)} монет из казны бригады!\n` +
    `💰 Осталось в казне: ${fmt(treasuryAmount - amount)} монет\n` +
    `👛 Твой баланс: ${fmt(p.balance + amount)} монет`
  );
}

// НОВОЕ: назначить зама
async function cmdSetDeputy(ctx, uid, targetIdStr) {
  const p = await getOrCreate(uid);
  if (p.banned) return send(ctx, '⛔ Вы заблокированы в игре и не можете использовать команды.');

  const targetId = parseInt(targetIdStr);
  if (!targetId) return send(ctx, '❌ Укажи ID игрока: «Зам [ID]»');

  if (!p.guild_id) return send(ctx, '❌ Ты не в бригаде.');

  const gr = await pool.query('SELECT * FROM guilds WHERE id=$1', [p.guild_id]);
  if (!gr.rows[0]) return send(ctx, '❌ Бригада не найдена.');
  
  const g = gr.rows[0];
  if (Number(g.owner_id) !== uid)
    return send(ctx, '❌ Только глава бригады может назначить заместителя.');
  ...
  await pool.query(
    `UPDATE guilds SET deputy_id=$1 WHERE id=$2`,
    [targetId, p.guild_id]
  );
  ...
  await send(ctx,
    ...
    `Теперь ${targetName} (ID: ${targetId}) может снимать монеты из казны бригады.`
  );

  try {
    await vk.api.messages.send({
      user_id: targetId,
      message: `👔 Ты назначен заместителем главы бригады!\nТеперь ты можешь использовать команду «Из казны [сумма]» для снятия монет.`,
      random_id: Math.floor(Math.random() * 1e9),
    });
  } catch {}
}
...
async function cmdRemoveDeputy(ctx, uid) {
  ...
  const g = gr.rows[0];
  if (Number(g.owner_id) !== uid)
    return send(ctx, '❌ Только глава бригады может снять заместителя.');

  if (!Number(g.deputy_id))
    return send(ctx, '❌ У бригады нет заместителя.');

  const deputyName = await getName(Number(g.deputy_id));
  await pool.query(
    `UPDATE guilds SET deputy_id=NULL WHERE id=$1`,
    [p.guild_id]
  );

  await send(ctx, `👔 ${deputyName} больше не заместитель главы бригады.`);
}
...
async function cmdLeaveGuild(ctx, uid) {
  ...
  // Если зам покидает — сбрасываем
  if (Number(g.deputy_id) === uid) {
...
async function cmdDisbandGuild(ctx, uid) {
  ...
  if (treasuryAmount > 0) {
    msg += `\n\n💰 Казна бригады (${fmt(treasuryAmount)} монет) переведена на ваш баланс!`;
  }
...
async function cmdDailyBonus(ctx, uid) {
  ...
  if (!p.guild_id)
    return send(ctx,
      ...
      `Вступи в бригаду или создай её: «Создать бригаду [имя]» (${fmt(GUILD_COST)} монет).`
    );
...
async function cmdBlackWork(ctx, uid) {
  ...
  if (caught) {
    ...
    msg += `\n💡 Совет: купи «Крышу» (${fmt(ROOF_COST)} монет), чтобы снизить риск в 2 раза!`;
    return send(ctx, msg);
  }
  ...
  msg += `\nПродать контрабанду: «Продать контрабанду» (${fmt(CONTRABAND_PRICE)} монет/шт.)\n`;
...
async function cmdTransfer(ctx, uid, args) {
  ...
  if (!targetId || isNaN(targetId) || !amount || amount < TRANSFER_MIN)
    return send(ctx,
      `❌ Неверный формат. Минимальный перевод: ${fmt(TRANSFER_MIN)} монет.\n` +
  ...
  try {
    await vk.api.messages.send({
      user_id: targetId,
      message: `💸 ${senderName} (ID: ${uid}) перевёл тебе ${fmt(amount)} монет!`,
      random_id: Math.floor(Math.random() * 1e9),
    });
  } catch {}
  ...
  await send(ctx,
    `✅ Перевод выполнен!\n` +
    `💸 ${fmt(amount)} монет → ${targetName} (ID: ${targetId})\n` +
    `👛 Твой баланс: ${fmt(p.balance - amount)} монет.`
  );
}
...
async function cmdMarketSell(ctx, uid, fullArgs) {
  ...
  if (args.length < 3) {
    return send(ctx,
      ...
      'Или: «Продать на рынке [название] [кол-во] [цена]» (для руд и предметов)\n\n' +
      'Примеры:\n' +
      ...
      '• Продать на рынке предмет динамит 2 300\n' +
      '• Продать на рынке вышка нефтяная 1 20000\n' +
      '• Продать на рынке железная пластина 3 200'
    );
  }

  let itemType, itemName, quantity, price;

  const firstArg = args[0].toLowerCase();

  if (['руда', 'предмет', 'бур', 'усиленный', 'алмазный', 'вышка'].includes(firstArg)) {
    if (firstArg === 'усиленный' && args[1]?.toLowerCase() === 'бур') {
      itemType = 'усиленный бур';
      itemName = 'усиленный бур';
      quantity = parseInt(args[2]);
      price = parseInt(args[3]);
    } else if (firstArg === 'алмазный' && args[1]?.toLowerCase() === 'бур') {
      itemType = 'алмазный бур';
      itemName = 'алмазный бур';
      quantity = parseInt(args[2]);
      price = parseInt(args[3]);
    } else if (firstArg === 'бур') {
      itemType = 'бур';
      itemName = args[1] || 'обычный бур';
      quantity = parseInt(args[2]);
      price = parseInt(args[3]);
    } else if (firstArg === 'руда') {
      itemType = 'руда';
      itemName = args[1];
      quantity = parseInt(args[2]);
      price = parseInt(args[3]);
    } else if (firstArg === 'предмет') {
      itemType = 'предмет';
      itemName = args.slice(1, -2).join(' ');
      quantity = parseInt(args[args.length - 2]);
      price = parseInt(args[args.length - 1]);
    } else if (firstArg === 'вышка') {
      itemType = 'вышка';
      itemName = args.slice(1, -2).join(' ') || 'нефтяная вышка';
      quantity = parseInt(args[args.length - 2]);
      price = parseInt(args[args.length - 1]);
    } else {
      itemType = firstArg;
    ...
    quantity = parseInt(args[args.length - 2]);
    price = parseInt(args[args.length - 1]);
    itemName = args.slice(0, -2).join(' ').toLowerCase();

    if (ORES[itemName]) {
      itemType = 'руда';
    } else if (itemName === 'контрабанда') {
      itemType = 'руда';
    } else if (itemName === 'обычный бур' || itemName === 'бур') {
      itemType = 'бур';
      ...
      itemType = 'алмазный бур';
    } else if (itemName.includes('вышка') || itemName === 'нефтяная вышка') {
      itemType = 'вышка';
  ...
  if (itemType === 'руда') {
    if (itemName === 'контрабанда') {
      const qty = await getItemQty(uid, 'контрабанда');
      if (qty < quantity) return send(ctx, `❌ У тебя только ${qty} шт. контрабанды.`);
      await removeItem(uid, 'контрабанда', quantity);
    } else if (ORES[itemName]) {
      ...
      const qty = await getItemQty(uid, itemName);
      if (qty < quantity) return send(ctx, `❌ У тебя только ${qty} шт. «${itemName}».`);
      await removeItem(uid, itemName, quantity);
    }
  ...
  } else if (itemType === 'усиленный бур') {
    ...
    return send(ctx, `❌ Неизвестный тип предмета: ${itemType}`);
  }
  ...
  await send(ctx,
    ...
    `📋 ${itemName} ×${quantity} по ${fmt(price)}💰/шт.\n` +
    `💰 Общая стоимость: ${fmt(quantity * price)}💰\n` +
    `🆔 Тип: ${itemType}`
  );
}
...
async function cmdMarketList(ctx) {
  ...
  if (!listings.rows.length)
    return send(ctx,
      ...
      'Или просто: «Продать на рынке алмаз 5 400»\n' +
  ...
  for (const lot of listings.rows) {
    ...
    lines.push(
      `#${lot.id} | ${lot.item_name} ×${lot.quantity} по ${fmt(Number(lot.price))}💰/шт.`,
      `   Продавец: ${sellerName} | Тип: ${lot.item_type}`,
      `   Всего: ${fmt(Number(lot.quantity) * Number(lot.price))}💰`,
      ''
    );
  }
...
async function cmdMarketBuy(ctx, uid, lotId) {
  ...
  if (itemType === 'руда' || itemType === 'предмет') {
    await addItem(uid, itemName, quantity);
  } else if (itemType === 'бур') {
    await pool.query(`UPDATE players SET drills=drills+$1 WHERE user_id=$2`, [quantity, uid]);
  } else if (itemType === 'усиленный бур') {
    await pool.query(`UPDATE players SET drills_enhanced=drills_enhanced+$1 WHERE user_id=$2`, [quantity, uid]);
  ...
  try {
    await vk.api.messages.send({
      ...
      message: `📦 Твой товар «${itemName}» ×${quantity} продан на рынке!\n💰 Получено: ${fmt(totalPrice)} монет\n👤 Покупатель: ${buyerName}`,
      random_id: Math.floor(Math.random() * 1e9),
    });
  } catch {}
  ...
  await send(ctx,
    `✅ Покупка совершена!\n` +
    `📦 ${itemName} ×${quantity} получено\n` +
    `💰 Потрачено: ${fmt(totalPrice)} монет\n` +
    `👤 Продавец: ${sellerName}`
  );
}
...
async function cmdListPromos(ctx) {
  ...
  for (const row of r.rows) {
    ...
    lines.push(`${status} ${row.code} — ${fmt(row.reward_amount)} ${row.reward_type} [${row.uses}/${row.max_uses}]`);
  }
  await send(ctx, lines.join('\n'));
}
...
async function cmdReferral(ctx, uid) {
  ...
  const refLink = `https://vk.com/write-225458430?ref=${p.referral_code}`;

  await send(ctx,
    `🔗 Твоя реферальная ссылка:\n${refLink}\n\n` +
    `👥 Приглашай друзей и получай бонусы!\n` +
    `💰 За каждого приглашённого: +${fmt(REFERRAL_REWARD_INVITER)} монет\n` +
    `🎁 Приглашённый получает: +${fmt(REFERRAL_REWARD_INVITED)} монет`
  );
}

// ИСПРАВЛЕНО: обработка рефералов через ref параметр
async function processReferral(refCode, newUserId) {
  ...
  // Проверяем, не пытается ли игрок использовать свою же рефку
...
// ═══════════════════════════════════════════════════════════
//  PvP МЕХАНИКИ — ИСПРАВЛЕНО: уведомления с названиями бригад
// ═══════════════════════════════════════════════════════════
...
async function cmdAcceptDuel(ctx, uid) {
  ...
  await send(ctx,
    ...
    `💰 Ставка ${fmt(bet)}💰 переходит победителю!${blockedMsg}\n\n` +
    `🔧 Проигравший может восстановить оборудование командой «Ремонт».`
  );

  try {
    await vk.api.messages.send({
      user_id: loser,
      message: `💀 Ты проиграл дуэль против ${winnerName}!\n💰 Потеряно: ${fmt(bet)} монет${blockedMsg}\n🔧 Восстанови оборудование: «Ремонт»`,
      random_id: Math.floor(Math.random() * 1e9),
    });
  } catch {}
}

// Рейды — ИСПРАВЛЕНО: уведомления всем участникам с названиями бригад
async function cmdRaid(ctx, uid, targetGuildName) {
  ...
  const treasuryAmount = Number(myGuild.treasury);
  if (treasuryAmount < RAID_COST)
    return send(ctx, `❌ В казне недостаточно средств. Нужно ${fmt(RAID_COST)}💰, в казне: ${fmt(treasuryAmount)}💰.`);
  ...
  // ИСПРАВЛЕНИЕ: рассылка всем участникам обеих бригад
  ...
  for (const pl of allAttackers.rows) {
    try {
      await vk.api.messages.send({
        user_id: pl.user_id,
        message: `🏴‍☠️ РЕЙД! Твоя бригада «${myGuild.name}» атакует «${targetGuildName}»!\n⏳ У тебя 1 час на диверсии: «Диверсия» (${fmt(SABOTAGE_COST)}💰)\n💰 Каждая диверсия наносит ${fmt(SABOTAGE_DAMAGE)} урона.`,
        random_id: Math.floor(Math.random() * 1e9),
      });
    } catch {}
  }
  ...
  await send(ctx,
    `🏴‍☠️ РЕЙД НАЧАТ!\n` +
    `⚔️ Атакует: «${myGuild.name}» → «${targetGuildName}»\n` +
    `⏳ Длительность: 1 час\n` +
    `💰 Из казны списано: ${fmt(RAID_COST)} монет\n\n` +
...
async function cmdSabotage(ctx, uid) {
  ...
  if (!p.guild_id) return send(ctx, '❌ Ты не состоишь в бригаде.');
  if (p.balance < SABOTAGE_COST) return send(ctx, `❌ Недостаточно монет. Нужно ${fmt(SABOTAGE_COST)}💰.`);
  ...
  // ИСПРАВЛЕНИЕ: показываем название вражеской бригады
  await send(ctx,
    `💣 Диверсия выполнена против бригады «${foundRaid.defenderName}»!\n` +
    `💰 Потрачено: ${fmt(SABOTAGE_COST)} монет\n` +
    `⚔️ Урон: +${fmt(SABOTAGE_DAMAGE)}\n` +
    `📊 Общий урон рейда: ${fmt(foundRaid.damage)}`
  );
}
...
async function cmdTrap(ctx, uid) {
  ...
  if (!p.guild_id) return send(ctx, '❌ Ты не состоишь в бригаде.');
  if (p.balance < TRAP_COST) return send(ctx, `❌ Недостаточно монет. Нужно ${fmt(TRAP_COST)}💰.`);
  ...
  await send(ctx,
    `🪤 Ловушка установлена против бригады «${foundRaid.attackerName}»!\n` +
    `💰 Потрачено: ${fmt(TRAP_COST)} монет\n` +
    `🛡️ Защита: +${fmt(SABOTAGE_DAMAGE)}\n` +
    `📊 Общая защита: ${fmt(foundRaid.traps)}`
  );
}

// ИСПРАВЛЕНИЕ: итоги рейда всем участникам
async function finishRaid(raidKey) {
  ...
  if (effectiveDamage > 0) {
    ...
    stolen = Math.min(stolen, RAID_MAX_STEAL, Number(defenderGuild.rows[0].treasury));
    ...
    resultMsg = `🏴‍☠️ РЕЙД ЗАВЕРШЁН!\n` +
      `⚔️ Бригада «${raid.attackerName}» успешно атаковала «${raid.defenderName}»!\n` +
      `💥 Нанесено урона: ${fmt(effectiveDamage)}\n` +
      `💰 Захвачено из казны: ${fmt(stolen)} монет`;
    ...
    for (const pl of allDefenders.rows) {
      try {
        await vk.api.messages.send({
          user_id: pl.user_id,
          message: `💀 Рейд завершён! Бригада «${raid.attackerName}» украла ${fmt(stolen)}💰 из казны!\n⚔️ Урон: ${fmt(effectiveDamage)}`,
          random_id: Math.floor(Math.random() * 1e9),
        });
      } catch {}
    }
    ...
    for (const pl of allAttackers.rows) {
      try {
        await vk.api.messages.send({
          user_id: pl.user_id,
          message: `🎉 РЕЙД УСПЕШЕН! Ваша бригада «${raid.attackerName}» разграбила «${raid.defenderName}»!\n💰 Захвачено: ${fmt(stolen)} монет в казну!\n⚔️ Нанесено урона: ${fmt(effectiveDamage)}`,
          random_id: Math.floor(Math.random() * 1e9),
        });
      } catch {}
    }
    ...
    resultMsg = `🛡️ РЕЙД ОТБИТ!\n` +
      `⚔️ Бригада «${raid.defenderName}» успешно защитилась от «${raid.attackerName}»!\n` +
      `💥 Заблокировано урона: ${fmt(raid.traps)}\n` +
      `💰 Компенсация: ${fmt(actualCompensation)} монет`;
    ...
    for (const pl of allAttackers.rows) {
      try {
        await vk.api.messages.send({
          user_id: pl.user_id,
          message: `❌ Рейд провален! Бригада «${raid.defenderName}» отбила атаку.\nВаша бригада теряет ${fmt(actualCompensation)}💰 компенсации.`,
          random_id: Math.floor(Math.random() * 1e9),
        });
      } catch {}
    }
    ...
    for (const pl of allDefenders.rows) {
      try {
        await vk.api.messages.send({
          user_id: pl.user_id,
          message: `🛡️ ЗАЩИТА УСПЕШНА! Вы отбили рейд бригады «${raid.attackerName}»!\n💰 Получено компенсации в казну: ${fmt(actualCompensation)} монет`,
          random_id: Math.floor(Math.random() * 1e9),
        });
      } catch {}
    }
...
// Снос, ремонт, война, шахта, перемирие
async function cmdDestroy(ctx, uid, targetIdStr, equipNumStr) {
  ...
  if (dynamiteQty <= 0)
    return send(ctx, `❌ Нужен динамит! Купи в магазине за ${fmt(DESTROY_COST)}💰: «Купить динамит»`);
  ...
  await send(ctx,
    ...
    `💰 Стоимость мгновенного ремонта: ${fmt(repairCost)} монет`
  );

  try {
    await vk.api.messages.send({
      user_id: targetId,
      message: `💥 Твой ${toDestroy.name} был уничтожен игроком ${await getName(uid)}!\n⏳ Восстановление через 6 часов или «Ремонт» за ${fmt(repairCost)}💰.`,
      random_id: Math.floor(Math.random() * 1e9),
    });
  } catch {}
}
...
async function cmdShowEquipment(ctx, uid, targetIdStr) {
  ...
  const dynamiteQty = await getItemQty(uid, 'динамит');
  if (dynamiteQty <= 0)
    return send(ctx, `❌ У тебя нет динамита! Купи в магазине: «Купить динамит» (${fmt(DESTROY_COST)}💰)`);
...
async function cmdRepair(ctx, uid) {
  ...
  for (const eq of destroyed.rows) {
    ...
    totalCost += cost;
    repairList.push(`${eq.equipment_type}: ${fmt(cost)}💰 (восстановится через ${fmtTime(eq.repair_until - now)})`);
  }
  ...
  if (p.balance < totalCost)
    return send(ctx,
      `🔧 РЕМОНТ ОБОРУДОВАНИЯ:\n\n` +
      repairList.join('\n') +
      `\n\n💰 Общая стоимость: ${fmt(totalCost)} монет\n` +
      `❌ Не хватает: ${fmt(totalCost - p.balance)} монет\n\n` +
  ...
  await send(ctx,
    ...
    repairList.join('\n') +
    `\n\n💰 Списано: ${fmt(totalCost)} монет\n` +
    `✅ Всё оборудование снова работает!`
  );
}

// Война — ИСПРАВЛЕНО: уведомления с названиями бригад
async function cmdWar(ctx, uid, targetGuildName) {
  ...
  if (Number(myGuild.rows[0].treasury) < WAR_COST)
    return send(ctx, `❌ В казне недостаточно средств. Нужно ${fmt(WAR_COST)}💰.`);
  ...
  activeWars.set(warKey, {
    ...
    name1: myGuild.rows[0].name,
    name2: targetGuildName,
    points1: 0,
    points2: 0,
    startTime: now,
  ...
  await send(ctx,
    `⚔️ ВОЙНА ОБЪЯВЛЕНА!\n\n` +
    `⚔️ «${myGuild.rows[0].name}» → «${targetGuildName}»\n` +
    `⏳ Длительность: 24 часа\n` +
    `💰 Из казны списано: ${fmt(WAR_COST)} монет\n\n` +
...
async function cmdAttack(ctx, uid, targetIdStr) {
  ...
  if (loserEquipment.length > 0) {
    ...
    blockedMsg = `\n💥 ${toBlock.name} заблокирован на ${fmtTime(WAR_BLOCK_DURATION)}`;
  }

  // ИСПРАВЛЕНИЕ: уведомление противнику при атаке
  try {
    await vk.api.messages.send({
      user_id: targetId,
      message: `⚔️ ТЕБЯ АТАКОВАЛИ!\nИгрок ${await getName(uid)} из бригады «${mySide === 'guild1' ? war.name1 : war.name2}» атаковал тебя!\n${winner === uid ? '💀 Ты проиграл эту стычку!' : '🏆 Ты победил в этой стычке!'}${blockedMsg}\n📊 Счёт войны: ${war.name1} ${war.points1} - ${war.points2} ${war.name2}`,
      random_id: Math.floor(Math.random() * 1e9),
    });
  } catch {}
  ...
  await send(ctx,
    `⚔️ АТАКА!\n` +
    `Ты атаковал ${await getName(targetId)} из бригады «${mySide === 'guild1' ? war.name2 : war.name1}»\n` +
    `${winner === uid ? '🏆 Ты победил!' : '💀 Ты проиграл!'}${blockedMsg}\n` +
    `📊 Счёт войны: ${war.name1} ${war.points1} - ${war.points2} ${war.name2}`
  );
}

// ИСПРАВЛЕНИЕ: итоги войны всем участникам
async function finishWar(warKey) {
  ...
  const winnerName = winnerGuildId === war.guild1 ? war.name1 : war.name2;
  const loserName = winnerGuildId === war.guild1 ? war.name2 : war.name1;
  ...
  for (const pl of loserPlayers.rows) {
    try {
      await vk.api.messages.send({
        user_id: pl.user_id,
        message: `💔 ВОЙНА ПРОИГРАНА!\n«${loserName}» проиграла «${winnerName}» со счётом ${war.points1}-${war.points2}\n📉 Штраф -10% к пассивному доходу на 24 часа.\n💰 Потеряно из казны: ${fmt(stealAmount)} монет`,
        random_id: Math.floor(Math.random() * 1e9),
      });
    } catch {}
  }

  // Рассылка победителям
  ...
  for (const pl of winnerPlayers.rows) {
    try {
      await vk.api.messages.send({
        user_id: pl.user_id,
        message: `🎉 ПОБЕДА В ВОЙНЕ!\n«${winnerName}» победила «${loserName}» со счётом ${war.points1}-${war.points2}\n🏰 Ваша бригада контролирует нейтральную шахту 3 дня (+20% к добыче)!\n💰 Захвачено из казны врага: ${fmt(stealAmount)} монет`,
        random_id: Math.floor(Math.random() * 1e9),
      });
    } catch {}
  }
...
async function cmdPeace(ctx, uid) {
  ...
  if (p.balance < PEACE_COST)
    return send(ctx, `❌ Недостаточно монет. Нужно ${fmt(PEACE_COST)}💰.`);
...
async function cmdHelp(ctx) {
  await send(ctx,
    ...
    `• Продать на рынке [предмет] [кол-во] [цена] — выставить лот\n` +
    `• Продать на рынке руда алмаз 5 400 — пример\n` +
    `• Купить на рынке [ID] — купить лот\n\n` +
    `👥 БРИГАДЫ:\n` +
    `• Бригада — информация о бригаде (с HP)\n` +
    `• Топ кланов — рейтинг бригад (с HP)\n` +
    `• Создать бригаду [имя] — создать\n` +
    `• Пригласить [ID] — пригласить игрока\n` +
    `• Принять приглашение — вступить\n` +
    `• Зам [ID] — назначить зама (глава)\n` +
    `• Снять зама — снять зама (глава)\n` +
    `• В казну [сумма] — пополнить казну\n` +
    `• Из казны [сумма] — снять из казны (глава или зам)\n` +
    ...
    `• Диверсия — диверсия при рейде (${fmt(SABOTAGE_COST)}💰)\n` +
    `• Ловушка — защита при рейде (${fmt(TRAP_COST)}💰)\n` +
    ...
    `• Перемирие — режим мира на 12 часов (${fmt(PEACE_COST)}💰)\n\n` +
    `🔗 РЕФЕРАЛКА:\n` +
    `• Реф / Реферал — твоя реферальная ссылка\n` +
    `• Приглашай друзей — получай ${fmt(REFERRAL_REWARD_INVITER)}💰, друг получит ${fmt(REFERRAL_REWARD_INVITED)}💰\n\n` +
    `🏆 РЕЙТИНГ:\n` +
...
async function cmdAdminHelp(ctx) {
  await send(ctx,
    ...
    `• !датьпредмет [ID] [предмет] [кол-во] — выдать предмет (бур, усиленный бур, алмазный бур, вышка и др.)\n` +
    ...
    `🔍 ИНФА:\n` +
    `• !датьинфу [ID] [1/7/30] — выдать доступ к «Инфа» на 1/7/30 дней\n` +
    `• !инфадоступ [ID] — проверить доступ игрока к «Инфа»\n\n` +
    `🦠 ВИРУС:\n` +
...
// ИСПРАВЛЕНИЕ: !датьпредмет с поддержкой буров/вышек
async function cmdAdminGiveItem(ctx, parts) {
  ...
  if (!targetId || !itemName) {
    return send(ctx, '❌ Использование: !датьпредмет [ID] [предмет] [кол-во]\nПоддерживаются: бур, усиленный бур, алмазный бур, вышка и обычные предметы');
  }
  
  await getOrCreate(targetId);
  const name = await getName(targetId);
  
  // ИСПРАВЛЕНИЕ: обработка разных типов предметов
  const normalizedItem = itemName.toLowerCase().trim();
  
  if (normalizedItem === 'бур' || normalizedItem === 'обычный бур') {
    await pool.query(`UPDATE players SET drills=drills+$1 WHERE user_id=$2`, [qty, targetId]);
    await send(ctx, `⚙️ Игроку ${name} (ID: ${targetId}) выдано: «Обычный бур» ×${qty}`);
  } else if (normalizedItem === 'усиленный бур' || normalizedItem === 'усиленный_бур') {
    await pool.query(`UPDATE players SET drills_enhanced=drills_enhanced+$1 WHERE user_id=$2`, [qty, targetId]);
    await send(ctx, `🔩 Игроку ${name} (ID: ${targetId}) выдано: «Усиленный бур» ×${qty}`);
  } else if (normalizedItem === 'алмазный бур' || normalizedItem === 'алмазный_бур') {
    await pool.query(`UPDATE players SET drills_diamond=drills_diamond+$1 WHERE user_id=$2`, [qty, targetId]);
    await send(ctx, `💎 Игроку ${name} (ID: ${targetId}) выдано: «Алмазный бур» ×${qty}`);
  } else if (normalizedItem === 'вышка' || normalizedItem === 'нефтяная вышка' || normalizedItem === 'нефтяная_вышка') {
    await pool.query(`UPDATE players SET oil_rigs=oil_rigs+$1 WHERE user_id=$2`, [qty, targetId]);
    await send(ctx, `🛢️ Игроку ${name} (ID: ${targetId}) выдано: «Нефтяная вышка» ×${qty}`);
  } else {
...
async function cmdAdminGive(ctx, parts) {
  ...
  await pool.query(`UPDATE players SET balance=balance+$1 WHERE user_id=$2`, [amount, targetId]);
  const name = await getName(targetId);
  await send(ctx, `✅ Игроку ${name} (ID: ${targetId}) выдано ${fmt(amount)} монет.`);
}
...
async function cmdAdminTake(ctx, parts) {
  ...
  const name = await getName(targetId);
  await send(ctx, `✅ У игрока ${name} (ID: ${targetId}) изъято ${fmt(actualTake)} монет.`);
}
...
async function cmdAdminTakeItem(ctx, parts) {
  ...
  // ИСПРАВЛЕНИЕ: обработка разных типов предметов при изъятии
  const normalizedItem = itemName.toLowerCase().trim();
  
  if (normalizedItem === 'бур' || normalizedItem === 'обычный бур') {
    const p = await getPlayer(targetId);
    if (!p) return send(ctx, `❌ Игрок #${targetId} не найден.`);
    const actualTake = Math.min(qty, p.drills);
    if (actualTake <= 0) return send(ctx, `❌ У игрока #${targetId} нет обычных буров.`);
    await pool.query(`UPDATE players SET drills=drills-$1 WHERE user_id=$2`, [actualTake, targetId]);
    const name = await getName(targetId);
    return send(ctx, `⚙️ У игрока ${name} (ID: ${targetId}) изъято: «Обычный бур» ×${actualTake}`);
  } else if (normalizedItem === 'усиленный бур') {
    const p = await getPlayer(targetId);
    if (!p) return send(ctx, `❌ Игрок #${targetId} не найден.`);
    const actualTake = Math.min(qty, p.drills_enhanced);
    if (actualTake <= 0) return send(ctx, `❌ У игрока #${targetId} нет усиленных буров.`);
    await pool.query(`UPDATE players SET drills_enhanced=drills_enhanced-$1 WHERE user_id=$2`, [actualTake, targetId]);
    const name = await getName(targetId);
    return send(ctx, `🔩 У игрока ${name} (ID: ${targetId}) изъято: «Усиленный бур» ×${actualTake}`);
  } else if (normalizedItem === 'алмазный бур') {
    const p = await getPlayer(targetId);
    if (!p) return send(ctx, `❌ Игрок #${targetId} не найден.`);
    const actualTake = Math.min(qty, p.drills_diamond);
    if (actualTake <= 0) return send(ctx, `❌ У игрока #${targetId} нет алмазных буров.`);
    await pool.query(`UPDATE players SET drills_diamond=drills_diamond-$1 WHERE user_id=$2`, [actualTake, targetId]);
    const name = await getName(targetId);
    return send(ctx, `💎 У игрока ${name} (ID: ${targetId}) изъято: «Алмазный бур» ×${actualTake}`);
  } else if (normalizedItem === 'вышка' || normalizedItem === 'нефтяная вышка') {
    const p = await getPlayer(targetId);
    if (!p) return send(ctx, `❌ Игрок #${targetId} не найден.`);
    const actualTake = Math.min(qty, p.oil_rigs);
    if (actualTake <= 0) return send(ctx, `❌ У игрока #${targetId} нет вышек.`);
    await pool.query(`UPDATE players SET oil_rigs=oil_rigs-$1 WHERE user_id=$2`, [actualTake, targetId]);
    const name = await getName(targetId);
    return send(ctx, `🛢️ У игрока ${name} (ID: ${targetId}) изъято: «Нефтяная вышка» ×${actualTake}`);
  }
...
async function cmdAdminPlayers(ctx) {
  ...
  const lines = [
    ...
    `📊 Всего монет в игре: ${fmt(totalWealth)}`,
  ...
  for (let i = 0; i < res.rows.length; i++) {
    ...
    lines.push(
      `${i + 1}. [id${r.user_id}|${name}] — ${fmt(Number(r.balance))}💰${classInfo}${guildInfo}${techInfo}${banStatus}`
    );
  }
...
async function cmdResetAllConfirm(ctx) {
  ...
  try {
    ...
    await pool.query(`UPDATE guilds SET treasury = 0, xp = 0, level = 1, war_points = 0, deputy_id = NULL`);
...
// ═══════════════════════════════════════════════════════════
//  ОБРАБОТЧИК СООБЩЕНИЙ — ИСПРАВЛЕНИЕ: рефералы при старте
// ═══════════════════════════════════════════════════════════
...
vk.updates.on('message_new', async (ctx) => {
  try {
    ...
    // ИСПРАВЛЕНИЕ: проверяем реферальный код из текста ссылки
    // Формат: vk.com/write-225458430?ref=КОД
    // При первом сообщении проверяем ref параметр
    const refMatch = raw.match(/[?&]ref=([A-Z0-9]+)/i);
    if (refMatch && refMatch[1]) {
      const refCode = refMatch[1].toUpperCase();
      // Проверяем, не обработан ли уже этот игрок
      const existingPlayer = await getPlayer(uid);
      if (!existingPlayer || !existingPlayer.referred_by) {
        await processReferral(refCode, uid);
      }
    }
    ...
    if (adminCheck) {
      ...
      if (parts[0] === '!датьинфу')          return cmdAdminGiveInfo(ctx, uid, parts[1], parts[2]);
      if (parts[0] === '!инфадоступ')        return cmdAdminCheckInfo(ctx, uid, parts[1]);
    ...
    // Инфа клан
    if (text.startsWith('инфа клан ') || text.startsWith('инфаклан ')) {
      const guildName = raw.slice(raw.indexOf(' ') + 1).trim();
      const cleanName = guildName.replace(/^клан\s+/i, '').trim();
      return cmdInfoGuild(ctx, uid, cleanName);
    }
    ...
    if (text.startsWith('инфа ') && !text.startsWith('инфа клан')) {
      ...
      if (mentionMatch) {
        targetIdStr = mentionMatch[1];
      }
      if (targetIdStr.startsWith('@')) {
        try {
          const screenName = targetIdStr.slice(1);
          const userInfo = await vk.api.users.get({ user_ids: [screenName] });
          if (userInfo && userInfo[0]) {
            targetIdStr = String(userInfo[0].id);
          }
        } catch {}
      }
      const targetId = parseInt(targetIdStr);
      if (targetId) {
        return cmdInfo(ctx, uid, targetId);
      }
    }
    ...
    // НОВОЕ: назначить зама
    if (text.startsWith('зам '))
      return cmdSetDeputy(ctx, uid, parseInt(text.slice('зам '.length).trim()));
    ...
    if (text.startsWith('продать на рынке '))
      return cmdMarketSell(ctx, uid, raw.slice('продать на рынке '.length).trim());
    ...
    switch (text) {
      ...
      // Замы
      case 'снять зама':
      case 'снять заместителя':   return cmdRemoveDeputy(ctx, uid);
...
(async () => {
  try {
    ...
    console.log('  1. Реферальная система — обрабатывает ref параметр при первом сообщении');
    console.log('  2. !датьпредмет — корректно выдаёт буры/вышки (drills, drills_enhanced, drills_diamond, oil_rigs)');
    console.log('  3. Формат баланса — везде toLocaleString("ru-RU") (18.000.000)');
    console.log('  4. Война и рейды — уведомления всем участникам с названиями бригад');
    console.log('  5. Замы — поле deputy_id в guilds, доступ к казне');
    console.log('  6. Классы, крафт, вирусы, PvP, рынок, рефералка — всё работает');
  } catch (err) {
