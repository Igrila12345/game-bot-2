...
// ═══════════════════════════════════════════════════════════
// АТОМАРНАЯ ЗАЩИТА ОТ СПАМА (НЕ ОТВЕЧАЕТ НА СПАМ)
// ═══════════════════════════════════════════════════════════
...
async function cmdDynamite(ctx, uid, targetId) {
    ...
    // Атомарная проверка КД и списание средств
    const result = await executeQuery(
        `UPDATE players 
...
async function cmdLotteryBuy(ctx, uid) {
    ...
    // Атомарное списание и запись билета
    const l = await executeQuery('SELECT * FROM lottery WHERE active = TRUE LIMIT 1');
    if (!l.rows[0]) return ctx.send('🎟️ Нет активной лотереи.');
    ...
    // Списание, запись билета и обновление счетчика в одной транзакции
...
vk.updates.on('message_new', async (ctx) => {
    ...
    try {
        ...
        try {
            ...
            // Единая атомарная проверка КД для всех команд
