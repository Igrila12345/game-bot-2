...
// ═══════════════════════════════════════════════════════════
// АТОМАРНАЯ ЗАЩИТА ОТ СПАМА
// ═══════════════════════════════════════════════════════════

async function checkAndUpdateCooldown(uid) {
    const now = getMskTimestamp();
    const result = await executeQuery(
        `UPDATE players 
         SET last_command = CASE 
             WHEN last_command IS NULL OR last_command < $2 THEN $1 
             ELSE last_command 
         END 
         WHERE user_id = $3 
         RETURNING last_command, 
                   CASE WHEN last_command IS NULL OR last_command < $2 THEN TRUE ELSE FALSE END as allowed`,
        [now, now - 3, uid]
    );
    
    if (!result.rows[0]) return { allowed: false, remaining: 3 };
    
    return { 
        allowed: result.rows[0].allowed,
        remaining: result.rows[0].allowed ? 0 : Math.max(0, 3 - (now - result.rows[0].last_command))
    };
}
...
async function cmdDynamite(ctx, uid, targetId) {
    ...
    if (!result.rows[0]) {
        ...
        return ctx.send('❌ Ошибка использования динамита.');
    }
...
async function cmdRat(ctx, uid, targetId) {
    ...
    if (!result.rows[0]) {
        ...
        return ctx.send('❌ Ошибка использования крысы.');
    }
...
async function cmdShield(ctx, uid) {
    ...
    if (!result.rows[0]) {
        ...
        if (p.banned) return ctx.send('⛔ Вы забанены.');
        return ctx.send('❌ Недостаточно средств (нужно 25 000 🍕).');
    }
    await ctx.send(`🛡️ Щит активирован!\n⏱ До: ${new Date(result.rows[0].shield_until * 1000).toLocaleTimeString('ru-RU', { timeZone: 'Europe/Moscow' })} МСК\n💰 Баланс: ${result.rows[0].balance} 🍕`);
}
...
async function cmdInfo(ctx, uid, text) {
    ...
    if (p.vip_until <= now) {
        return ctx.send('❌ Команда /info доступна только VIP-игрокам!\n💡 Купить VIP: донат купить вип');
    }
    ...
    if (targetId) {
        ...
        if (!target) return ctx.send('❌ Игрок не найден.');
        if (target.banned) return ctx.send('❌ Игрок забанен.');
        if (target.hidden) return ctx.send('❌ Игрок скрыт.');
        ...
        return ctx.send([
            ...
            `💎 Донат-баланс: ${(target.donate_balance || 0).toLocaleString()} ₿`,
    ...
    return ctx.send([
        `👤 Ваш профиль: @id${uid} (vk.com/id${uid})`,
        ``,
        `💡 Используйте: /info @user — посмотреть чужой профиль`
    ].join('\n'));
}
...
async function cmdDonateBuy(ctx, uid, item) {
    ...
    if ((p.donate_balance || 0) < s.price_rub) {
        return ctx.send(`❌ Недостаточно донат-баланса! Нужно: ${s.price_rub} ₿, у вас: ${(p.donate_balance || 0).toLocaleString()} ₿`);
    }
...
vk.updates.on('message_new', async (ctx) => {
    ...
    try {
        ...
        try {
            ...
            // Проверка бана
            if (player && player.banned && lower !== 'старт') {
                userLocks.delete(uid);
                return ctx.send('⛔ Вы забанены.');
            }

            // Проверка КД только для зарегистрированных игроков
            if (player && !(player.no_cd_until > getMskTimestamp())) {
                const cdCheck = await checkAndUpdateCooldown(uid);
                if (!cdCheck.allowed) {
                    userLocks.delete(uid);
                    return; // Игнорируем при спаме
                }
            }

            // Админ-команды (без КД)
            if (uid === ADMIN_ID && (text.startsWith('!') || text.startsWith('/рассылка'))) {
                const result = await cmdAdmin(ctx, uid, text);
                userLocks.delete(uid);
                return result;
            }

            // Команда старт доступна без регистрации
            if (lower === 'старт') {
                await getOrCreate(uid);
                userLocks.delete(uid);
            ...
            if (!player) {
                userLocks.delete(uid);
                return await ctx.send('❌ Сначала напишите "старт" для регистрации.');
            }

            if (lower === 'хелп' || lower === 'помощь') {
                const result = await cmdHelp(ctx);
                userLocks.delete(uid);
                return result;
            }
            if (lower === 'профиль') {
                const result = await cmdProfile(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
            if (lower.startsWith('/info') || lower.startsWith('инфо')) {
                const result = await cmdInfo(ctx, uid, text);
                userLocks.delete(uid);
                return result;
            }

            if (lower === 'бонус' || lower === 'daily' || lower === 'ежедневный') {
                const result = await cmdDailyBonus(ctx, uid);
                userLocks.delete(uid);
                return result;
            }

            // Пагинация карт
            if (lower === 'карты' || lower === 'коллекция') {
                const result = await cmdCards(ctx, uid, 1);
                userLocks.delete(uid);
                return result;
            }
            if (lower.startsWith('карты ')) {
                ...
                if (parts[1] === 'слот') {
                    ...
                    if (!slot || !cardId) {
                        userLocks.delete(uid);
                        return ctx.send('❌ карты слот [1-3] [ID карты]');
                    }
                    const result = await cmdCardSet(ctx, uid, slot, cardId);
                    userLocks.delete(uid);
                    return result;
                }
                if (parts[1] === 'шансы') {
                    const result = await cmdCardsChances(ctx);
                    userLocks.delete(uid);
                    return result;
                }
                const page = parseInt(parts[1]);
                if (!isNaN(page) && page > 0) {
                    const result = await cmdCards(ctx, uid, page);
                    userLocks.delete(uid);
                    return result;
                }
                userLocks.delete(uid);
            ...
            if (lower === 'скилл' || lower === 'навыки' || lower === 'скиллы') {
                const result = await cmdSkills(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
            if (lower.startsWith('скилл купить')) {
                ...
                if (!tree || !level) {
                    userLocks.delete(uid);
                    return ctx.send('❌ скилл купить [buy/dynamite/rat/special] [уровень]');
                }
                const result = await cmdSkillBuy(ctx, uid, tree, level);
                userLocks.delete(uid);
                return result;
            }
            ...
            if (lower.startsWith('купить печь')) {
                ...
                const result = await cmdBuyOven(ctx, uid, num);
                userLocks.delete(uid);
                return result;
            }
            ...
            if (lower.startsWith('донат купить')) {
                ...
                const result = await cmdDonateBuy(ctx, uid, item);
                userLocks.delete(uid);
                return result;
            }

            if (lower === 'купить билет' || lower === 'лотерея купить') {
                const result = await cmdLotteryBuy(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
            ...
            if (lower.startsWith('купить')) {
                ...
                const result = await cmdBuy(ctx, uid, count);
                userLocks.delete(uid);
                return result;
            }

            if (lower === 'печи') {
                const result = await cmdOvens(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
            ...
            if (lower.startsWith('динамит')) {
                ...
                if (!tid) {
                    userLocks.delete(uid);
                    return await ctx.send('❌ динамит @user');
                }
                const result = await cmdDynamite(ctx, uid, tid);
                userLocks.delete(uid);
                return result;
            }
            ...
            if (lower.startsWith('крыса')) {
                ...
                if (!tid) {
                    userLocks.delete(uid);
                    return await ctx.send('❌ крыса @user');
                }
                const result = await cmdRat(ctx, uid, tid);
                userLocks.delete(uid);
                return result;
            }

            if (lower === 'щит') {
                const result = await cmdShield(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
            ...
            if (lower.startsWith('клан создать')) {
                ...
                const result = await cmdClanCreate(ctx, uid, name);
                userLocks.delete(uid);
                return result;
            }
            if (lower.startsWith('клан вступить')) {
                const name = text.slice('клан вступить'.length).trim();
                const result = await cmdClanJoin(ctx, uid, name);
                userLocks.delete(uid);
                return result;
            }
            if (lower === 'клан удалить') {
                const result = await cmdClanDelete(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
            if (lower === 'клан выйти') {
                const result = await cmdClanLeave(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
            if (lower === 'клан список') {
                const result = await cmdClanList(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
            if (lower === 'клан') {
                const result = await cmdClanInfo(ctx, uid);
                userLocks.delete(uid);
                return result;
            }

            if (lower === 'топ баланс') {
                const result = await cmdTopBalance(ctx);
                userLocks.delete(uid);
                return result;
            }
            if (lower === 'топ кланов') {
                const result = await cmdTopClans(ctx);
                userLocks.delete(uid);
                return result;
            }
            if (lower === 'топ') {
                const result = await cmdTop(ctx);
                userLocks.delete(uid);
                return result;
            }

            if (lower === 'лотерея') {
                const result = await cmdLotteryInfo(ctx);
                userLocks.delete(uid);
                return result;
            }
            ...
            if (lower.startsWith('промокод')) {
                ...
                const result = await cmdPromo(ctx, uid, code);
                userLocks.delete(uid);
                return result;
            }

            if (lower === 'донат') {
                const result = await cmdDonate(ctx, uid);
                userLocks.delete(uid);
                return result;
            }
