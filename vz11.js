...
// Нужно для корректного получения from_id и reply_message в беседах через Long Poll
...
async function handleCommand(msg) {
  ...
  // Игнорируем сообщения от ботов и нулевые id
  if (!uid || uid <= 0) return
...
async function longPoll() {
  ...
  try {
    ...
    for (const update of res.updates || []) {
      ...
      // Пропускаем исходящие и уже обработанные
      if (isOut || processedIds.has(msgId) || msgId <= lastMsgId) continue

      processedIds.add(msgId)
      lastMsgId = Math.max(lastMsgId, msgId)

      // Получаем полное сообщение через API — это ключевое исправление
      // В беседах Long Poll не передаёт from_id напрямую
