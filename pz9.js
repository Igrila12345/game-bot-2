...
async function cmdInfo(ctx, uid, text) {
    ...
    if (subCmd === 'отключить' || subCmd === 'выключить' || subCmd === 'скрыть' || subCmd === 'off') {
        await executeQuery('UPDATE players SET info_enabled = FALSE WHERE user_id = $1', [uid]);
        return ctx.send('🔒 Ваш профиль скрыт!');
    }
    
    if (subCmd === 'включить' || subCmd === 'показать' || subCmd === 'on') {
        await executeQuery('UPDATE players SET info_enabled = TRUE WHERE user_id = $1', [uid]);
...
vk.updates.on('message_new', async (ctx) => {
    try {
        ...
        if (lower.startsWith('/info') || lower.startsWith('инфо')) return cmdInfo(ctx, uid, text);
        ...
        if (lower.startsWith('динамит')) {
            const tid = extractId(text);
            if (!tid) return ctx.send('❌ динамит @user');
            return cmdDynamite(ctx, uid, tid);
        }
        
        if (lower.startsWith('крыса')) {
            const tid = extractId(text);
            if (!tid) return ctx.send('❌ крыса @user');
