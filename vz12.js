...
const TOKEN = 'vk1.a.pmWhcwfFSHq_5r9UsOc9J3uSeDzZssl77XJ4kAJfPsc1IFbPMir6aHOhRr5DgtesLeCieiYOw-JbNKtG4u6V7nzFKbfTTf5Aa0-_wmwGxlOcwiD1RMQxieYoiEwQbnCsppujzPC-EYD0KQ-KzWja3MCs1mX3fnGGhv1kojrAnAL-iBc-tJ8eWpr9DoFB_weL-SsePCBFGQzddH5upydxnw'
...
function flipText(text) {
  const map = {
    'а': 'ɐ', 'б': 'ƍ', 'в': 'ʚ', 'г': 'ɹ', 'д': 'ɓ', 'е': 'ǝ', 'ё': 'ǝ̤', 'ж': 'ж', 'з': 'ε', 'и': 'и', 'й': 'й',
...
// ========== ПОЛУЧЕНИЕ СООБЩЕНИЙ ЧЕРЕЗ messages.getById ==========
async function getFullMessage(msgId) {
  try {
    const r = await api('messages.getById', { message_ids: msgId, extended: 0 })
    if (r.error || !r.response?.items?.length) return null
...
async function handleCommand(msg) {
  ...
  if (rpTrigger) {
    if (relationActions[rpTrigger]) {
      ...
      if (!rel) rel = await addRelationHp(uid, rep.from_id, 1, uid, 'начало отношений')

      const newRel = await addRelationHp(uid, rep.from_id, action.hp, uid, rpTrigger)
      const level = getRelationLevel(newRel.hp)

      if (action.cooldown > 0) lastActionTime.set(coolKey, Date.now())
  ...
  if (cmd === '!помощь' || cmd === '!хелп' || cmd === '!help' || cmd === '!команды') {
    ...
    const hasModPerms = await hasPerm(peer, uid, 'manage')
    const totalPages = (isOwner || hasModPerms) ? 4 : 3

    let help = ''
    switch (page) {
      case 1:
        help = `📋 Помощь [1/${totalPages}] — Отношения и общение
...
📄 Следующая: !помощь 2`
      ...
      case 2:
        help = `📋 Помощь [2/${totalPages}] — Экономика
...
📄 Следующая: !помощь 3`
      ...
      case 3:
        help = `📋 Помощь [3/${totalPages}] — Профиль и социальное
...
!пинг — проверка работы бота`
        ...
        if ((isOwner || hasModPerms)) {
          help += '\n\n📄 Следующая: !помощь 4 (модерация)'
        }
        break

      case 4:
        if (!(isOwner || hasModPerms)) {
          return sendMsg(peer, '❌ Страница недоступна')
        }
        help = `📋 Помощь [4/${totalPages}] — Модерация
...
📄 Предыдущая: !помощь 3`
      ...
      default:
        return sendMsg(peer, `❌ Страница ${page} не найдена. Доступно: 1-${totalPages}`)
    }
  ...
  if (cmd === '-отн' || cmd === '!-отн' || cmd === '!отн разорвать' || cmd === '!отн расстаться') {
    ...
    if (!targetId && args[1]) { const match = args[1].match(/id(\d+)/); if (match) targetId = parseInt(match[1]) }
  ...
  if (cmd === '!отн' && (args[1] === 'статус' || args[1] === 'стата')) {
    const rep = msg.reply_message
    let targetId = rep ? rep.from_id : null
    if (!targetId && args[2]) { const match = args[2].match(/id(\d+)/); if (match) targetId = parseInt(match[1]) }
  ...
  if (cmd === '!админы' || cmd === '!staff' || cmd === '!управляющие' || cmd === '!admins' || cmd === '!ктоадмин') {
    ...
    if (ranks.rows.length) {
      for (const rank of ranks.rows) {
        ...
        if (members.rows.length) { for (const m of members.rows) { const name = await getUserName(m.user_id); result += `  🟢 @id${m.user_id} (${name})\n` } }
        else result += '  Нет участников\n'
        result += '\n'
      }
    } else {
  ...
  if (cmd === '!рпсоздать' || cmd === '!создатьрп') {
    ...
    if (parts.length < 2) return sendMsg(peer, '❌ !рпсоздать Название | эмодзи | действие\n{user} — отправитель, {target} — цель, {random} — случайный\nСтоимость: 100💰')
  ...
  if (cmd === '!рпудалить' || cmd === '!удалитьрп') {
    ...
    if (c.rows[0].created_by !== uid && !await hasPerm(peer, uid, 'manage')) return sendMsg(peer, '❌ Это не ваша команда')
  ...
  if (cmd === '!создатьранг' || cmd === '!добавитьранг') {
    if (!await hasPerm(peer, uid, 'manage')) return sendMsg(peer, '⛔ Нет прав')
    const parts = txt.split('|').map(p => p.trim())
  ...
  // ===== МОДЕРАЦИЯ =====
  if (cmd === '!бан') {
    if (!await hasPerm(peer, uid, 'ban')) return sendMsg(peer, '⛔ Нет прав')
    const rep = msg.reply_message
    if (!rep) return sendMsg(peer, '❌ Ответьте на сообщение')
    await pg.query('INSERT INTO blacklist VALUES ($1,$2) ON CONFLICT DO NOTHING', [rep.from_id, 'Бан'])
    if (peer > 2e9) try { await api('messages.removeChatUser', { chat_id: peer - 2e9, user_id: rep.from_id }) } catch {}
    return sendMsg(peer, `🔨 @id${rep.from_id} забанен`)
  }

  if (cmd === '!кик') {
    if (!await hasPerm(peer, uid, 'kick')) return sendMsg(peer, '⛔ Нет прав')
    const rep = msg.reply_message
    if (!rep) return sendMsg(peer, '❌ Ответьте на сообщение')
    if (peer > 2e9) try { await api('messages.removeChatUser', { chat_id: peer - 2e9, user_id: rep.from_id }) } catch {}
    return sendMsg(peer, `👢 @id${rep.from_id} кикнут`)
  }

  if (cmd === '!мут') {
    if (!await hasPerm(peer, uid, 'mute')) return sendMsg(peer, '⛔ Нет прав')
    const rep = msg.reply_message
    if (!rep) return sendMsg(peer, '❌ Ответьте на сообщение')
    const min = parseInt(args[1]) || 10
    await pg.query('INSERT INTO mutes VALUES ($1,$2,$3) ON CONFLICT (peer_id,user_id) DO UPDATE SET until=$3', [peer, rep.from_id, Date.now() / 1000 + min * 60])
    return sendMsg(peer, `🤐 @id${rep.from_id} мут ${min} мин`)
  }

  if (cmd === '!размут' || cmd === '!анмут') {
    if (!await hasPerm(peer, uid, 'mute')) return sendMsg(peer, '⛔ Нет прав')
    const rep = msg.reply_message
    if (!rep) return sendMsg(peer, '❌ Ответьте на сообщение')
    await pg.query('DELETE FROM mutes WHERE peer_id=$1 AND user_id=$2', [peer, rep.from_id])
    return sendMsg(peer, `🔊 @id${rep.from_id} размучен`)
  }

  if (cmd === '!варн' || cmd === '!пред') {
    if (!await hasPerm(peer, uid, 'warn')) return sendMsg(peer, '⛔ Нет прав')
    const rep = msg.reply_message
    if (!rep) return sendMsg(peer, '❌ Ответьте на сообщение')
    const w = await pg.query('SELECT count FROM warns WHERE peer_id=$1 AND user_id=$2', [peer, rep.from_id])
    const c = (w.rows[0]?.count || 0) + 1
    await pg.query('INSERT INTO warns VALUES ($1,$2,$3) ON CONFLICT (peer_id,user_id) DO UPDATE SET count=$3', [peer, rep.from_id, c])
    if (c >= 3 && peer > 2e9) {
      try { await api('messages.removeChatUser', { chat_id: peer - 2e9, user_id: rep.from_id }) } catch {}
      await pg.query('DELETE FROM warns WHERE peer_id=$1 AND user_id=$2', [peer, rep.from_id])
      return sendMsg(peer, `🔨 @id${rep.from_id} кикнут (3/3)`)
    }
    return sendMsg(peer, `⚠️ @id${rep.from_id} ${c}/3`)
  }

  if (cmd === '!стата' || cmd === '!статистика') {
    try {
      const r = await api('messages.getConversationMembers', { peer_id: peer })
      return sendMsg(peer, `📊 Участников: ${r.response?.count || 0}`)
    } catch { return sendMsg(peer, '❌ Ошибка') }
  }

  // ===== КЛАНЫ =====
  if (cmd === '!клансоздать' || cmd === '!кланкрейт') {
    const nm = args.slice(1).join(' ')
    if (!nm) return sendMsg(peer, '❌ !клансоздать [название]')
    if (await getClan(uid)) return sendMsg(peer, '❌ Уже в клане')
    if (await getBal(uid) < 500) return sendMsg(peer, '❌ Нужно 500💰')
    await addBal(uid, -500)
    const res = await pg.query('INSERT INTO clans (name, owner_id) VALUES ($1,$2) RETURNING id', [nm, uid])
    await pg.query('INSERT INTO clan_members VALUES ($1,$2,$3)', [res.rows[0].id, uid, 'owner'])
    return sendMsg(peer, `🏰 Клан "${nm}" создан!`)
  }

  if (cmd === '!клан' || cmd === '!мойклан') {
    const cl = await getClan(uid)
    if (!cl) return sendMsg(peer, '❌ Не в клане')
    const ms = await pg.query('SELECT user_id, role FROM clan_members WHERE clan_id=$1', [cl.id])
    return sendMsg(peer, `🏰 ${cl.name}\n💰 ${cl.balance}\n\nУчастники:\n${ms.rows.map(m => `@id${m.user_id} (${m.role})`).join('\n')}`)
  }

  if (cmd === '!кланпринять' || cmd === '!кланэдд') {
    const cl = await getClan(uid)
    if (!cl || cl.role !== 'owner') return sendMsg(peer, '❌ Только владелец')
    const rep = msg.reply_message
    if (!rep || await getClan(rep.from_id)) return sendMsg(peer, '❌')
    await pg.query('INSERT INTO clan_members VALUES ($1,$2,$3)', [cl.id, rep.from_id, 'member'])
    return sendMsg(peer, `✅ @id${rep.from_id} принят в клан`)
  }

  if (cmd === '!кланвыйти' || cmd === '!кланлив') {
    const cl = await getClan(uid)
    if (!cl || cl.role === 'owner') return sendMsg(peer, '❌ Владелец не может выйти')
    await pg.query('DELETE FROM clan_members WHERE clan_id=$1 AND user_id=$2', [cl.id, uid])
    return sendMsg(peer, '👋 Вы вышли из клана')
  }
}
...
async function initLongPoll() {
  try {
    ...
    longPollServer = res.response.server
    longPollKey = res.response.key
    longPollTs = res.response.ts
    console.log('✅ Long Poll готов')
    return true
...
async function longPoll() {
  ...
  try {
    ...
    if (res.failed) {
      console.log('LP failed:', res.failed)
      if (res.failed === 2 || res.failed === 3) await initLongPoll()
      else longPollTs = res.ts
      return
    }
    ...
    for (const update of res.updates || []) {
      ...
      const msgId = update[1]
      const flags = update[2]
      const isOut = (flags & 2) !== 0

      if (isOut || processedIds.has(msgId) || msgId <= lastMsgId) continue

      processedIds.add(msgId)
      lastMsgId = Math.max(lastMsgId, msgId)

      getFullMessage(msgId).then(fullMsg => {
        if (!fullMsg) return
        if (!fullMsg.from_id || fullMsg.from_id <= 0) return

        const msg = {
          id: fullMsg.id,
          peer_id: fullMsg.peer_id,
          from_id: fullMsg.from_id,
          text: fullMsg.text || '',
          reply_message: fullMsg.reply_message || null,
          out: fullMsg.out === 1
        }

        if (!msg.out) {
          handleCommand(msg).catch(err => console.error('handleCommand err:', err))
        }
      }).catch(err => console.error('getFullMessage err:', err))
    }

    if (processedIds.size > 500) {
      const arr = Array.from(processedIds).sort((a, b) => a - b)
      for (let i = 0; i < arr.length - 200; i++) processedIds.delete(arr[i])
    }
  } catch (err) {
    console.error('LP loop error:', err)
  }
}
