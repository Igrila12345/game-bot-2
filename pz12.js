...
async function checkCommandCooldown(uid) {
    ...
    if (memoryCd && now - memoryCd < 3) return false;
    playerCooldowns.set(uid, now);
    await executeQuery('UPDATE players SET last_command = $1 WHERE user_id = $2', [now, uid]).catch(() => {});
    return true;
}
...
async function cmdCards(ctx, uid, page = 1) {
    ...
    msg.push(``);
    msg.push(`📦 Все карты (страница ${page}):`);

    const perPage = 10;
    const startIdx = (page - 1) * perPage;
    const allCardsArr = Object.entries(ALL_CARDS).slice(startIdx, startIdx + perPage);
    ...
    if (totalPages > 1) {
        msg.push(``);
        msg.push(`📄 Страница ${page}/${totalPages} | карты ${page + 1}`);
    }
...
async function cmdDailyBonus(ctx, uid) {
    ...
    if (p.last_daily_bonus >= yesterdayStart && p.last_daily_bonus < todayStart) {
        streak = (streak % 7) + 1;
    } else if (p.last_daily_bonus < yesterdayStart || streak === 0) {
        streak = 1;
    ...
    // Выдаём награду атомарно
    switch (reward.type) {
        case 'coins':
            await executeQuery(
                'UPDATE players SET balance = balance + $1, last_daily_bonus = $2, daily_streak = $3 WHERE user_id = $4',
                [reward.amount, now, streak, uid]
            );
            break;
        case 'pizza':
            await executeQuery(
                'UPDATE players SET pizzerias = LEAST($1, pizzerias + $2), last_daily_bonus = $3, daily_streak = $4 WHERE user_id = $5',
                [MAX_PIZZERIAS, reward.amount, now, streak, uid]
            );
        ...
        case 'shield': {
            ...
            await executeQuery(
                'UPDATE players SET shield_until = $1, last_daily_bonus = $2, daily_streak = $3 WHERE user_id = $4',
                [newShieldEnd, now, streak, uid]
            );
            break;
        }
        case 'vip': {
            ...
            await executeQuery(
                'UPDATE players SET vip_until = $1, last_daily_bonus = $2, daily_streak = $3, daily_week_rewarded = TRUE WHERE user_id = $4',
                [newVipEnd, now, streak, uid]
            );
            break;
        }
    }
...
async function cmdShield(ctx, uid) {
    if (!await checkCommandCooldown(uid)) return ctx.send('⏱ Подождите 3 секунды между командами!');
    const p = await getOrCreate(uid);
    if (p.banned) return ctx.send('⛔ Вы забанены.');
...
vk.updates.on('message_new', async (ctx) => {
    ...
    try {
        ...
        try {
            ...
            if (lower === 'карты' || lower === 'коллекция') return await cmdCards(ctx, uid);
            if (lower.startsWith('карты слот')) {
                const parts = text.trim().split(/\s+/);
            ...
            if (lower === 'карты шансы') return await cmdCardsChances(ctx);
