...
async function getOrCreate(uid) {
  await pool.query(`
    ...
    VALUES ($1, 0, 100, 1, 0, 0, 0, 0, 0, 0, 0, 0, FALSE, 0, 0, FALSE, FALSE, 0, 0, 0, NULL, FALSE)
...
// НОВАЯ: Админ-команда !игроки - показывает ВСЕХ игроков
...
async function cmdAdminTakeOre(ctx, parts) {
  ...
  await pool.query(
    `UPDATE inventory SET quantity=quantity-$1 WHERE user_id=$2 AND item=$3`,
    [actualTake, targetId, oreName]
  );
...
// НОВАЯ: Изъятие предмета админом
...
vk.updates.on('message_new', async (ctx) => {
  try {
    ...
    // ── Админ-команды (главный админ и назначенные админы) ──
