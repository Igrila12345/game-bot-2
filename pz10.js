...
async function cmdInfo(ctx, uid, text) {
    ...
    if (subCmd === 'отключить' || subCmd === 'выключить' || subCmd === 'скрыть' || subCmd === 'off') {
        ...
        return ctx.send('🔒 Ваш профиль скрыт!');
    }
    ...
    if (subCmd === 'удалить' || subCmd === 'delete') {
        const p = await getPlayer(uid);
        if (p && p.clan_id && p.clan_role === 'leader') {
            const clanId = p.clan_id;
            await executeQuery('UPDATE players SET clan_id = NULL, clan_role = $1 WHERE clan_id = $2', ['member', clanId]);
            await executeQuery('DELETE FROM clans WHERE id = $1', [clanId]);
        }
        
        await executeQuery('DELETE FROM players WHERE user_id = $1', [uid]);
        await executeQuery('DELETE FROM promo_uses WHERE user_id = $1', [uid]);
        await executeQuery('DELETE FROM lottery_players WHERE user_id = $1', [uid]);
        return ctx.send('🗑️ Аккаунт удалён. Напишите "старт" чтобы начать заново.');
    }
...
async function cmdClanJoin(ctx, uid, name) {
    ...
    // Если лидер возвращается - восстановить права
...
async function cmdClanDelete(ctx, uid) {
    ...
    // Удаляем всех участников из клана
...
async function cmdDonateBuy(ctx, uid, item) {
    ...
    if (!shop.rows[0]) return ctx.send('❌ Товар не найден. Ключи: баланс_100к, баланс_500к, баланс_1м, пиццерии_50, пиццерии_200, пиццерии_500, печь3, щит, вип\n\nСписок: донат');
...
async function cmdAdmin(ctx, uid, text) {
    ...
    if (cmd === '!admin') {
        return ctx.send([
            ...
            '',
            '⚡ ИВЕНТЫ:',
            '• !ивенты вкл — включить ивенты',
            '• !ивенты выкл — выключить ивенты',
            '• !ивенты — статус',
            '',
            '📊 ПРОЧЕЕ:',
            '• !стат — статистика',
    ...
    if (cmd === '!лотерея') {
        ...
        if (subCmd === 'создать') {
            ...
            // Деактивируем старые лотереи
            await executeQuery('UPDATE lottery SET active = FALSE WHERE active = TRUE');
            
            // Создаём новую
            await executeQuery(
                'INSERT INTO lottery (price, slots, prize_type, prize_amount, created_at) VALUES ($1,$2,$3,$4,$5)',
                [price, slots, prizeType, prizeAmount, getMskTimestamp()]
            );
            
            const prizeText = prizeType === 'pizza' 
                ? `${prizeAmount || price * slots} пиццерий` 
                : `${prizeAmount || price * slots} 🍕`;
...
vk.updates.on('message_new', async (ctx) => {
    ...
    try {
        ...
        // Защита от одновременных команд
