...
// ═══════════════════════════════════════════════════════════
// КД И ЗАЩИТА ОТ СПАМА (НЕ ОТВЕЧАЕТ НА СПАМ)
// ═══════════════════════════════════════════════════════════
const playerCooldowns = new Map();

function checkGlobalCooldown(uid) {
    const now = getMskTimestamp();
    const lastTime = playerCooldowns.get(uid);
...
function updateCooldown(uid) {
    const now = getMskTimestamp();
    playerCooldowns.set(uid, now);
    executeQuery('UPDATE players SET last_command = $1 WHERE user_id = $2', [now, uid]).catch(() => {});
}
...
async function cmdInfo(ctx, uid, text) {
    ...
    // /info доступна ТОЛЬКО VIP-игрокам
    if (p.vip_until <= now) {
    ...
    // Если указан пользователь - показываем его профиль
    if (targetId) {
        const target = await getPlayer(targetId);
        if (!target) return ctx.send('❌ Игрок не найден.');
        if (target.banned) return ctx.send('❌ Игрок забанен.');
    ...
    // Если пользователь не указан - показываем свой профиль
    return ctx.send([
...
async function cmdTop(ctx) {
    const r = await executeQuery('SELECT user_id, pizzerias, income FROM players WHERE banned = FALSE ORDER BY pizzerias DESC LIMIT 10');
...
async function cmdTopBalance(ctx) {
    const r = await executeQuery('SELECT user_id, balance FROM players WHERE banned = FALSE ORDER BY balance DESC LIMIT 10');
    if (!r.rows.length) return ctx.send('📊 Рейтинг пока пуст.');
...
async function cmdAdmin(ctx, uid, text) {
    ...
    if (cmd === '!геймеры') {
        const players = await executeQuery('SELECT user_id, balance, pizzerias, donate_balance, banned FROM players ORDER BY user_id');
        if (!players.rows.length) return ctx.send('📋 Нет игроков.');
        
        const lines = ['👥 ВСЕ ИГРОКИ:', ''];
        for (const p of players.rows) {
            const status = p.banned ? '🚫' : '✅';
            lines.push(`${status} @id${p.user_id} | 🍕 ${p.balance.toLocaleString()} | 🏠 ${p.pizzerias} шт | 💎 ${p.donate_balance.toLocaleString()}₿`);
        }
...
vk.updates.on('message_new', async (ctx) => {
    ...
    try {
        ...
        if (!isNoCd) {
            const cdCheck = checkGlobalCooldown(uid);
            if (!cdCheck.allowed) {
        ...
        if (userLocks.get(uid)) {
            return; // Игнорируем если команда ещё выполняется
        }
        ...
        try {
            ...
            // Обновляем КД только для успешных команд
            if (!isNoCd) {
                updateCooldown(uid);
            }
