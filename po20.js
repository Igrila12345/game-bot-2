...
async function initDB() {
  ...
  await pool.query(`
    ...
    ALTER TABLE players ADD COLUMN IF NOT EXISTS pickaxe_lvl INT DEFAULT 1;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS drills INT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS oil_rigs INT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS last_work BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS last_rest BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS last_drill BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS last_oil BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS guild_id INT DEFAULT NULL;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS last_black BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS jail_until BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS has_roof BOOLEAN DEFAULT FALSE;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS last_steal BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS last_daily BIGINT DEFAULT 0;
    ...
    ALTER TABLE players ADD COLUMN IF NOT EXISTS virus INT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS virus_end BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS vaccine_end BIGINT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS player_class VARCHAR(32) DEFAULT NULL;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS is_admin BOOLEAN DEFAULT FALSE;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS work_count INT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS craft_level INT DEFAULT 1;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS drills_enhanced INT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS drills_diamond INT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS battle_rating INT DEFAULT 0;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS referral_code VARCHAR(16) UNIQUE;
    ALTER TABLE players ADD COLUMN IF NOT EXISTS referred_by BIGINT DEFAULT NULL;
  `);
...
// НОВОЕ: команда "Инфа" — просмотр чужого профиля
async function cmdInfo(ctx, uid, targetId) {
  ...
  const lines = [
    `🔍 ПРОФИЛЬ: ${name}`,
    `🆔 ID: ${targetId}`,
    `💰 Баланс: ${p.balance.toLocaleString()} монет`,
    `❤️ Стамина: ${p.stamina}/${MAX_STAMINA}`,
    `⛏️ Кирка: ур. ${p.pickaxe_lvl} (+${pick.bonus}% к добыче)`,
    `⚔️ Боевой рейтинг: ${p.battle_rating}`,
    classInfo,
...
// НОВОЕ: команда "Инфа клан" — просмотр информации о клане
async function cmdInfoGuild(ctx, uid, guildName) {
  ...
  const lines = [
    ...
    `💰 Общий баланс участников: ${Number(totalWealth.rows[0].total).toLocaleString()} монет`,
    `👤 Участников: ${memberCount}`,
    `📈 XP: ${Number(g.xp)}/${nextCost} (до ур. ${guildLevel + 1})`,
  ...
  for (let i = 0; i < topMembers.rows.length; i++) {
    ...
    lines.push(`${i + 1}. ${mName} — ${Number(m.balance).toLocaleString()}💰`);
  }

  await send(ctx, lines.join('\n'));
}

// НОВОЕ: админ-команда выдачи доступа к "Инфа"
...
// НОВОЕ: админ-команда проверки доступа
...
async function cmdGuildTop(ctx) {
  ...
  for (let i = 0; i < res.rows.length; i++) {
    ...
    lines.push(
      `${placeEmoji} ${g.name} [Ур. ${level}] | ❤️HP: ${hp}`,
      `   👑 Глава: ${ownerName}`,
      `   👤 Участников: ${members}`,
...
async function cmdCraft(ctx, uid, craftName) {
  ...
  // ИСПРАВЛЕНИЕ: расширенный парсинг для пластины и других предметов
...
async function cmdMarketSell(ctx, uid, fullArgs) {
  ...
  // Парсим аргументы: тип, название, количество, цена
  // Примеры:
  // "руда алмаз 5 400"
  // "предмет динамит 2 300"
  // "бур обычный 1 5000"
  // "усиленный бур усиленный 1 8000"  
  // "алмазный бур алмазный 1 15000"
  // "вышка нефтяная 1 20000"
  // "железная пластина 3 200"
  ...
  // Проверяем, является ли первый аргумент типом
  if (['руда', 'предмет', 'бур', 'усиленный', 'алмазный', 'вышка'].includes(firstArg)) {
    ...
    // Без типа — автоопределение
    // Пытаемся угадать: последние два аргумента — количество и цена
    quantity = parseInt(args[args.length - 2]);
    price = parseInt(args[args.length - 1]);
    ...
    if (ORES[itemName]) {
      ...
      itemType = 'руда'; // Контрабанда продаётся как руда
  ...
  // Проверяем наличие предмета и списываем
  if (itemType === 'руда') {
    if (itemName === 'контрабанда') {
      ...
      // Может быть craft_resource или другой предмет
      const qty = await getItemQty(uid, itemName);
...
async function cmdRaid(ctx, uid, targetGuildName) {
  ...
  // ИСПРАВЛЕНИЕ: проверяем казну и списываем только если достаточно
  const treasuryAmount = Number(myGuild.treasury);
  if (treasuryAmount < RAID_COST)
    return send(ctx, `❌ В казне недостаточно средств. Нужно ${RAID_COST}💰, в казне: ${treasuryAmount}💰.`);
...
vk.updates.on('message_new', async (ctx) => {
  try {
    ...
    if (text.startsWith('инфа ') && !text.startsWith('инфа клан')) {
      ...
      // Поддержка упоминаний [id123|@durov]
...
(async () => {
  try {
    ...
    console.log('🔍 Система «Инфа»: игроки с доступом могут смотреть чужие профили и кланы');
    console.log('❤️ HP кланов: уровень * 1000 + участники * 100');
    console.log('📦 Рынок полностью переписан — поддержка всех типов предметов');
    console.log('🔨 Крафт пластины исправлен');
  } catch (err) {
