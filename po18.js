...
const STEAL_MAX_AMOUNT    = 100000; // Максимальный лимит кражи
...
async function getDailyAttackCount(uid) {
  ...
  let count = 0;
  for (const [attacker, target, time] of attackCounts) {
    if (attacker === uid && time >= today) count++;
  }
  return count;
}
...
async function getAvailableEquipment(uid) {
  ...
  if (p.drills > 0) {
    const destroyed = await pool.query(
      `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='drill' AND repair_until > $2`,
      [uid, now]
    );
    const destroyedCount = Number(destroyed.rows[0]?.cnt || 0);
  ...
  if (p.drills_enhanced > 0) {
    const destroyed = await pool.query(
      `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='drill_enhanced' AND repair_until > $2`,
      [uid, now]
    );
    const destroyedCount = Number(destroyed.rows[0]?.cnt || 0);
  ...
  if (p.drills_diamond > 0) {
    const destroyed = await pool.query(
      `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='drill_diamond' AND repair_until > $2`,
      [uid, now]
    );
    const destroyedCount = Number(destroyed.rows[0]?.cnt || 0);
  ...
  if (p.oil_rigs > 0) {
    const destroyed = await pool.query(
      `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='oil_rig' AND repair_until > $2`,
      [uid, now]
    );
    const destroyedCount = Number(destroyed.rows[0]?.cnt || 0);
...
async function cmdIncome(ctx, uid) {
  ...
  // Проверяем уничтоженные буры
  const destroyedDrills = await pool.query(
    `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='drill' AND repair_until > $2`,
    [uid, now]
  );
  const destroyedEnhanced = await pool.query(
    `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='drill_enhanced' AND repair_until > $2`,
    [uid, now]
  );
  const destroyedDiamond = await pool.query(
    `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='drill_diamond' AND repair_until > $2`,
    [uid, now]
  );

  const activeDrills = p.drills - Number(destroyedDrills.rows[0]?.cnt || 0);
  const activeEnhanced = p.drills_enhanced - Number(destroyedEnhanced.rows[0]?.cnt || 0);
  const activeDiamond = p.drills_diamond - Number(destroyedDiamond.rows[0]?.cnt || 0);

  if (activeDrills > 0) {
    let inc = DRILL_INCOME;
    if (engineerBonus > 0) inc = Math.floor(inc * (1 + engineerBonus));
    earned += Math.floor(hours * inc * activeDrills);
  }
  if (activeEnhanced > 0) {
    let inc = 100;
    if (engineerBonus > 0) inc = Math.floor(inc * (1 + engineerBonus));
    earned += Math.floor(hours * inc * activeEnhanced);
  }
  if (activeDiamond > 0) {
    let inc = 250;
    if (engineerBonus > 0) inc = Math.floor(inc * (1 + engineerBonus));
    earned += Math.floor(hours * inc * activeDiamond);
  }
  ...
  if (activeDrills > 0) {
    let inc = DRILL_INCOME;
    if (engineerBonus > 0) inc = Math.floor(inc * (1 + engineerBonus));
    msg += `• Обычные буры: ${activeDrills}/${p.drills} активны (+${activeDrills * inc}/ч)\n`;
  }
  if (activeEnhanced > 0) {
    let inc = 100;
    if (engineerBonus > 0) inc = Math.floor(inc * (1 + engineerBonus));
    msg += `• Усиленные буры: ${activeEnhanced}/${p.drills_enhanced} активны (+${activeEnhanced * inc}/ч)\n`;
  }
  if (activeDiamond > 0) {
    let inc = 250;
    if (engineerBonus > 0) inc = Math.floor(inc * (1 + engineerBonus));
    msg += `• Алмазные буры: ${activeDiamond}/${p.drills_diamond} активны (+${activeDiamond * inc}/ч)\n`;
  }
...
async function cmdOilIncome(ctx, uid) {
  ...
  // Проверяем уничтоженные вышки
  const destroyedRigs = await pool.query(
    `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='oil_rig' AND repair_until > $2`,
    [uid, now]
  );
  const activeRigs = p.oil_rigs - Number(destroyedRigs.rows[0]?.cnt || 0);

  if (activeRigs === 0)
    return send(ctx, `🛢️ Все твои вышки уничтожены! Восстанови их командой «Ремонт».`);
  ...
  let earned = Math.floor(hours * baseIncome * activeRigs);
  ...
  let msg = `🛢️ Доход с ${activeRigs}/${p.oil_rigs} вышки(ок)!\n💰 +${earned} монет (за ${hours.toFixed(1)} ч.)\n📈 Доход: ${activeRigs * baseIncome} монет/час`;
...
async function cmdProfile(ctx, uid) {
  ...
  // Проверяем уничтоженное оборудование
  const destroyedDrills = await pool.query(
    `SELECT COUNT(*) as cnt FROM destroyed_equipment WHERE owner_id=$1 AND equipment_type='drill' AND repair_until > $2`,
    [uid, now]
  );
  ...
  if (p.drills > 0) {
    const active = p.drills - Number(destroyedDrills.rows[0]?.cnt || 0);
    ...
    const drillPending = Math.floor(drillHours * baseDillIncome * active);
    passiveInfo += `\n⚙️ Обычные буры: ${active}/${p.drills} активны (+${active * baseDillIncome}/ч, накоп: ${drillPending} монет)`;
  }
  if (p.drills_enhanced > 0) {
    const active = p.drills_enhanced - Number(destroyedEnhanced.rows[0]?.cnt || 0);
    const drillHours = Math.min((now - p.last_drill) / 3_600_000, PASSIVE_CAP_HOURS);
    let inc = 100;
    if (getClassBonus(p, 'drill') > 0) inc = Math.floor(inc * 1.25);
    const pending = Math.floor(drillHours * inc * active);
    passiveInfo += `\n🔩 Усиленные буры: ${active}/${p.drills_enhanced} активны (+${active * inc}/ч, накоп: ${pending} монет)`;
  }
  if (p.drills_diamond > 0) {
    const active = p.drills_diamond - Number(destroyedDiamond.rows[0]?.cnt || 0);
    const drillHours = Math.min((now - p.last_drill) / 3_600_000, PASSIVE_CAP_HOURS);
    let inc = 250;
    if (getClassBonus(p, 'drill') > 0) inc = Math.floor(inc * 1.25);
    const pending = Math.floor(drillHours * inc * active);
    passiveInfo += `\n💎 Алмазные буры: ${active}/${p.drills_diamond} активны (+${active * inc}/ч, накоп: ${pending} монет)`;
  }
  if (p.drills === 0 && p.drills_enhanced === 0 && p.drills_diamond === 0) {
    passiveInfo += `\n⚙️ Буры: нет`;
  }
  if (p.oil_rigs > 0) {
    const active = p.oil_rigs - Number(destroyedRigs.rows[0]?.cnt || 0);
    ...
    const oilPending = Math.floor(oilHours * baseOilIncome * active);
    passiveInfo += `\n🛢️ Вышки: ${active}/${p.oil_rigs} активны (+${active * baseOilIncome}/ч, накоп: ${oilPending} монет)`;
  } else {
    passiveInfo += `\n🛢️ Вышки: нет`;
  }
...
async function cmdSteal(ctx, uid, targetIdStr) {
  ...
  // ЛИМИТ НА ВОРОВСТВО: максимум 100 000 монет
...
async function cmdGuildInfo(ctx, uid) {
  ...
  // Проверяем, контролирует ли бригада нейтральную шахту
  let mineInfo = '';
...
async function cmdMarketSell(ctx, uid, itemType, itemName, quantity, price) {
  ...
  if (itemType === 'руда') {
    ...
    if (!SHOP_ITEMS[itemName] && !CRAFT_RESOURCES[itemName])
      return send(ctx, `❌ Предмет «${itemName}» не найден.`);
    const qty = await getItemQty(uid, itemName);
    if (qty < quantity) return send(ctx, `❌ У тебя только ${qty} шт. предмета «${itemName}».`);
    await removeItem(uid, itemName, quantity);
  } else if (itemType === 'бур') {
    if (p.drills < quantity) return send(ctx, `❌ У тебя только ${p.drills} буров.`);
    ...
    return send(ctx, '❌ Неверный тип. Доступные: руда, предмет, бур, вышка');
  }
...
async function cmdReferral(ctx, uid) {
  ...
  const refLink = `https://vk.com/write-${ctx.$groupId || 'club225458430'}?ref=${p.referral_code}`;
...
async function cmdAcceptDuel(ctx, uid) {
  ...
  // Блокируем случайное оборудование проигравшего
...
async function finishRaid(raidKey) {
  ...
  if (effectiveDamage > 0) {
    ...
    const defGuild = defenderGuild.rows[0];
    ...
    // Защитники отбили атаку
    const compensation = 5000;
    await pool.query(`UPDATE guilds SET treasury=treasury+$1 WHERE id=$2`, [compensation, raid.defenderGuild]);
    await pool.query(`UPDATE guilds SET treasury=GREATEST(0, treasury-$1) WHERE id=$2`, [compensation, raid.attackerGuild]);
    ...
    for (const pl of allAttackers.rows) {
      try {
        await vk.api.messages.send({
          user_id: pl.user_id,
          message: `❌ Рейд провален! Защитники отбили атаку. Ваша бригада теряет ${compensation}💰 компенсации.`,
          random_id: Math.floor(Math.random() * 1e9),
        });
      } catch {}
    }
...
async function cmdDestroy(ctx, uid, targetIdStr, equipNumStr) {
  ...
  // Проверяем кулдаун на уничтожение этой цели
  const lastDestroy = await pool.query(
    `SELECT MAX(destroyed_at) as last FROM destroyed_equipment WHERE owner_id=$1`,
    [targetId]
  );
  if (lastDestroy.rows[0]?.last > Date.now() - DESTROY_COOLDOWN)
    return send(ctx, `❌ Этого игрока можно атаковать не чаще раза в сутки.`);
...
async function finishWar(warKey) {
  ...
  if (winnerGuildId) {
    ...
    const stealAmount = Math.min(
      Math.floor(Number(loserGuild.treasury) * WAR_WIN_PCT),
      WAR_WIN_MAX
    );
    ...
    // Передача контроля над нейтральной шахтой
    ...
    const penaltyEnd = Date.now() + WAR_LOSE_PENALTY_DUR;
...
// ИСПРАВЛЕННАЯ команда !датьпредмет
async function cmdAdminGiveItem(ctx, parts) {
  const targetId = parseInt(parts[1]);
  const itemName = parts[2]?.toLowerCase();
  const qty = parseInt(parts[3]) || 1;
  
  // Проверяем ВСЕ возможные предметы: SHOP_ITEMS, CRAFT_RESOURCES, и просто строки
  if (!targetId || !itemName || (!SHOP_ITEMS[itemName] && !CRAFT_RESOURCES[itemName] && !ORES[itemName] && itemName !== 'контрабанда' && itemName !== 'динамит' && itemName !== 'еда' && itemName !== 'крыша' && itemName !== 'аптечка' && itemName !== 'вакцина' && itemName !== 'антибиотик')) {
    // Если предмет не найден нигде, всё равно выдаём (на случай кастомных предметов)
    await getOrCreate(targetId);
    await addItem(targetId, itemName, qty);
    const name = await getName(targetId);
    return send(ctx, `📦 Игроку ${name} (ID: ${targetId}) выдано: «${itemName}» ×${qty}`);
  }
...
// НОВАЯ КОМАНДА: обнуление прогресса всех игроков
async function cmdResetAll(ctx) {
  ...
  // Флаг ожидания подтверждения можно хранить в памяти
  if (!global.pendingResets) global.pendingResets = new Set();
  global.pendingResets.add(ctx.senderId);
}

async function cmdResetAllConfirm(ctx) {
  if (!global.pendingResets || !global.pendingResets.has(ctx.senderId)) {
    return send(ctx, '❌ Сначала используйте команду !обнулитьвсех для предупреждения.');
  }

  global.pendingResets.delete(ctx.senderId);

  try {
    // Обнуляем баланс, стамину, кирку, буры, вышки, рейтинг, уровень крафта
    await pool.query(`
      UPDATE players SET
        balance = 0,
        stamina = 100,
...
vk.updates.on('message_new', async (ctx) => {
  try {
    ...
    if (text.startsWith('продать на рынке ')) {
      ...
      return cmdMarketSell(ctx, uid, parts[0], parts[1], parseInt(parts[2]), parseInt(parts[3]));
    }
...
(async () => {
  try {
    ...
    console.log('👑 Админ-команда !обнулитьвсех добавлена');
  } catch (err) {
