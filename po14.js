...
async function getOrCreate(uid) {
  await pool.query(`
    INSERT INTO players (user_id, balance, stamina, pickaxe_lvl, drills, oil_rigs,
                         ...
                         hidden, banned, virus, virus_end, vaccine_end, player_class)
    VALUES ($1, 0, 100, 1, 0, 0, 0, 0, 0, 0, 0, 0, FALSE, 0, 0, FALSE, FALSE, 0, 0, 0, NULL)
...
// Получить бонус класса для игрока
...
async function cmdStart(ctx, uid) {
  ...
  // Новый игрок - показываем выбор класса
...
async function cmdWork(ctx, uid) {
  ...
  if (p.guild_id) {
    ...
    if (gr.rows[0]) {
      ...
      // Проверяем, есть ли в бригаде бригадир
  ...
  await pool.query(
    `UPDATE players SET balance=balance+$1, stamina=$2, last_work=$3 WHERE user_id=$4`,
  ...
  if (oreLines.length > 0) {
    msg += `\n🎒 Добытые руды:\n${oreLines.join('\n')}\n`;
    msg += `(«Продать всё» — обменять на монеты)\n`;
  } else {
    msg += `\n🎒 Руда не попалась на этот раз.\n`;
  }
...
async function cmdIncome(ctx, uid) {
  ...
  if (p.drills === 0)
    return send(ctx, `⚙️ У тебя нет буров.\nКупи бур командой «Бур [кол-во]» за ${DRILL_COST} монет/шт.`);

  const now = Date.now();
  const hours = Math.min((now - p.last_drill) / 3_600_000, PASSIVE_CAP_HOURS);
  let baseIncome = DRILL_INCOME;
  
  // Бонус инженера
  const engineerBonus = getClassBonus(p, 'drill');
  if (engineerBonus > 0) {
    baseIncome = Math.floor(baseIncome * (1 + engineerBonus));
  }
  
  let earned = Math.floor(hours * baseIncome * p.drills);
  ...
  let msg = `⚙️ Доход с ${p.drills} бур(ов)!\n💰 +${earned} монет (за ${hours.toFixed(1)} ч.)\n📈 Доход: ${p.drills * baseIncome} монет/час`;
  if (engineerBonus > 0) msg += `\n⚙️ Бонус инженера: +25%`;
...
async function cmdOilIncome(ctx, uid) {
  ...
  if (p.oil_rigs === 0)
    return send(ctx, `🛢️ У тебя нет нефтяных вышек.\nКупи командой «Нефть [кол-во]» за ${OIL_COST} монет/шт.`);
...
async function cmdProfile(ctx, uid) {
  ...
  if (p.drills > 0) {
    ...
    passiveInfo += `\n⚙️ Буры: ${p.drills} шт. (+${p.drills * baseDillIncome}/ч, накоп: ${drillPending} монет)`;
  } else {
    passiveInfo += `\n⚙️ Буры: нет`;
  }
  if (p.oil_rigs > 0) {
...
async function cmdDisbandGuild(ctx, uid) {
  ...
  // Сохраняем сумму казны
  const treasuryAmount = Number(g.treasury);

  // Снимаем всех участников с бригады
  await pool.query(
...
async function cmdSteal(ctx, uid) {
  const p = await getOrCreate(uid);
  if (p.banned) return send(ctx, '⛔ Вы заблокированы в игре и не можете использовать команды.');
  ...
  const topRes = await pool.query(
    `SELECT user_id, balance FROM players
     WHERE user_id != $1 AND balance >= $2 AND banned = FALSE
  ...
  const victimBalance = Number(victim.balance);
  const victimId = Number(victim.user_id);
  ...
  if (!success) {
    ...
    // Уведомление жертве о неудачной попытке кражи
    try {
      await vk.api.messages.send({
        user_id: victimId,
  ...
  // Уведомление жертве об успешной краже
  try {
    await vk.api.messages.send({
      user_id: victimId,
...
async function cmdAdminHelp(ctx) {
  await send(ctx,
    ...
    `👤 ИГРОКИ:\n` +
    `• !игроки — список всех игроков (топ 30)\n` +
    ...
    `• !бан [ID] — забанить игрока (запретить все действия)\n` +
...
async function cmdAdminPlayers(ctx) {
  const res = await pool.query(
    'SELECT user_id, balance, banned, player_class FROM players ORDER BY balance DESC LIMIT 30'
  );
  if (!res.rows.length) return send(ctx, '📊 Игроков пока нет.');
  const lines = ['👤 Все игроки (топ 30):'];
  for (const r of res.rows) {
    const name = await getName(r.user_id);
    const banStatus = r.banned ? ' [ЗАБАНЕН]' : '';
    const classInfo = r.player_class ? ` (${r.player_class})` : '';
    lines.push(`${r.user_id}. [id${r.user_id}|${name}] (@id${r.user_id}) — ${Number(r.balance)} монет${classInfo}${banStatus}`);
  }
  await send(ctx, lines.join('\n'));
}
...
vk.updates.on('message_new', async (ctx) => {
  try {
    ...
    switch (text) {
      ...
      case 'своровать':           return cmdSteal(ctx, uid);
